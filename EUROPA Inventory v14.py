...
CREW_SIZE = 4
TOTAL_MONTHS = 10 # 9 months mission + 1 month redundancy
DAYS_IN_MISSION = TOTAL_MONTHS * 30.44 # Use 30.44 as the average days per month

# --- BARCODE MAPPING (Reverted to 12-digit codes) ---
...
# --- INITIAL INVENTORY (UPDATED TO REFLECT CONDIMENT CHANGE) ---
def get_initial_inventory():
    """
    Calculates initial stock.
    Core food (4 categories) stock is calculated based on CREW_SIZE and DAYS_IN_MISSION (10 months total).
    Condiments stock is linked to the total core food units and does not influence core calculations.
    """
    
    # 1. CORE FOOD CALCULATION (Hydratable, Thermostabilized, Natural, Desserts)
    # Target: 4 crew * 304.4 days * ~12 units/day (simplified rate) = 14,611 total units. Rounding up to 14,800.
    TOTAL_CORE_UNITS_REQUIRED = 14800 
    
    # Distribute total required units across the 4 core categories (40 items total)
    target_units_per_category = TOTAL_CORE_UNITS_REQUIRED / 4 
    base_stock_per_item_core = int(np.ceil(target_units_per_category / 10)) # ~370 units per item if distributed evenly

    def calculate_core_stock(multiplier):
        # Calculate the required stock for one CORE item
        return int(np.ceil(base_stock_per_item_core * multiplier))
        
    # --- Multipliers for the 4 core categories (Kept the same ratios as before) ---
    ...
    # Item names from the existing BARCODE_MAP (ordered by their original 12-digit code prefix)
    items_hm = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101890")] 
    items_tm = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101891")]
    items_nf = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101892")]
    items_db = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101893")]
    
    # 2. CONDIMENTS STOCK (Set high, linked to core food totals)
    # Target: Condiments should be roughly 15-20% of the total core food unit count (14800) for high-volume items.
    TOTAL_CONDIMENT_UNITS_REQUIRED = 2500 # Approximately 17% of core food units (14800)
    
    # Base stock per item for condiments (10 items total)
    base_stock_per_item_condiment = int(np.ceil(TOTAL_CONDIMENT_UNITS_REQUIRED / 10)) # 250 units per item if distributed evenly

    def calculate_condiment_stock(multiplier):
        # Calculate the required stock for one CONDIMENT item
        # Use a high multiplier factor (e.g., 20) to ensure the stock is high and the final numbers are large.
        return int(np.ceil(base_stock_per_item_condiment * multiplier / 5)) 

    # Condiment multipliers (adjusted for larger numbers but maintaining variety ratio)
    stock_cs = [5.5, 5.0, 8.0, 8.0, 3.5, 3.2, 2.8, 4.5, 7.0, 3.5]
    items_cs = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101894")]


    inventory_data = {
        "Hydratable Meals": {
            name: {"current": calculate_core_stock(mult), "original": calculate_core_stock(mult)}
            for name, mult in zip(items_hm, stock_hm)
        },
        "Thermostabilized Meals": {
            name: {"current": calculate_core_stock(mult), "original": calculate_core_stock(mult)}
            for name, mult in zip(items_tm, stock_tm)
        },
        "Natural Form & Irradiated": {
            name: {"current": calculate_core_stock(mult), "original": calculate_core_stock(mult)}
            for name, mult in zip(items_nf, stock_nf)
        },
        "Desserts & Beverages (Non-Mix)": {
            name: {"current": calculate_core_stock(mult), "original": calculate_core_stock(mult)}
            for name, mult in zip(items_db, stock_db)
        },
        "Condiments & Spreads": {
            name: {"current": calculate_condiment_stock(mult), "original": calculate_condiment_stock(mult)}
            for name, mult in zip(items_cs, stock_cs)
        }
    }
...
# --- DATA PREP FOR DASHBOARD (Condiments are Excluded from core food totals) ---
def create_inventory_dataframe():
    """Converts inventory data into a Pandas DataFrame for Altair plotting and calculates metrics."""
    data_list = []
    total_original_food = 0
    total_current_food = 0
    low_stock_count = 0
    zero_stock_count = 0

    # List of categories considered 'core food' for mission life calculations (Condiments is excluded)
    ...
    for category, items in st.session_state.inventory.items():
        ...
        # Days Remaining calculation only applies to CORE FOOD categories
        if category in CORE_FOOD_CATEGORIES:
            # Add to the running total for overall mission life calculation
            total_current_food += cat_current
            total_original_food += cat_original
            
            # Days remaining calculation: Total stock / (Total original stock / Total mission days)
            days_remaining = np.floor(cat_current / (cat_original / DAYS_IN_MISSION)) if cat_original > 0 else np.inf
            consumption_rate = (cat_original - cat_current) / cat_original if cat_original > 0 else 0
        else:
            # Condiments don't contribute to 'days remaining' in the same way, set to N/A
        ...
        data_list.append({
            ...
            'Is_Core_Food': category in CORE_FOOD_CATEGORIES
        })

    df = pd.DataFrame(data_list)
    
    # Calculate overall total days remaining based on ALL core food
    if total_original_food > 0:
        overall_days_remaining = (total_current_food / total_original_food) * DAYS_IN_MISSION
    else:
        overall_days_remaining = 0

    # Total units includes ALL categories (food + condiments)
    total_current = sum(df['Current Stock'])
    total_original = sum(df['Original Stock'])
    
    return df, total_current, total_original, low_stock_count, zero_stock_count, overall_days_remaining
...
def dashboard_page():
    ...
    df, total_current, total_original, low_stock_count, zero_stock_count, overall_days_remaining = create_inventory_dataframe()

    # --- Global Metrics Section ---
    st.markdown("<h3>Mission-Wide Status</h3>", unsafe_allow_html=True)
    
    # ADDED new metric for Overall Days Remaining
    ...
    col2.metric("Low Stock Items (<=30)", low_stock_count, delta=f"{low_stock_delta} low items", delta_color="off" if low_stock_delta == 0 else "inverse")
    ...
    # NEW: Overall Days Remaining
    # Use the core food totals for this crucial metric
    col5.metric(
        "Est. Total Days Left (Food)", 
        f"{int(overall_days_remaining):,}", 
        delta=f"Mission Target: {int(DAYS_IN_MISSION):,} days"
    )

    st.markdown("<br>", unsafe_allow_html=True) 

    # --- Key Status Indicators (New Multi-Row Display with fixed titles) ---
    st.markdown("<h3>Key Status Indicators (By Category)</h3>", unsafe_allow_html=True)

    # Create 5 rows of metrics, one for each category
    for i, category in enumerate(df['Category']):
        data = df[df['Category'] == category].iloc[0]
        
        # Determine the color and icon for Days Remaining
        days_rem = data['Days Remaining']
        icon = ICON_MAP.get(category, '•')

        if data['Is_Core_Food']:
            # For Core Food
            if days_rem <= 30:
                delta_color = "inverse"
                status_text = "CRITICAL: <30 Days"
            elif days_rem <= 60:
                delta_color = "off" # Use off for amber warning color
                status_text = "WARNING: <60 Days"
            else:
                delta_color = "normal"
                status_text = "NOMINAL"
            days_rem_display = f"{int(days_rem):,}"
        else:
            # For Condiments
            delta_color = "normal"
            status_text = "NON-CALORIC SUPPLY"
            days_rem_display = "N/A"
            
        
        # Use columns for a single, focused row for each category
        col_cat_title, col_units, col_days, col_status = st.columns([1.8, 1, 1, 1.2]) 
        
        with col_cat_title:
            # Displaying the full category name
            st.metric(f"Category: {category}", f"{icon} {category}")

        with col_units:
            # Current units against original capacity
            st.metric("Units Left", f"{data['Current Stock']:,}", delta=f"Max: {data['Original Stock']:,}")
            
        with col_days:
            # Days Remaining (Key metric)
            st.metric("Est. Days Left", days_rem_display, delta=status_text, delta_color=delta_color)
            
        with col_status:
            # Consumption Rate (as delta against 0)
            st.metric("Consumption %", f"{data['Consumption Rate']:.1%}", delta=f"Used: {data['Consumed Stock']:,}", delta_color="off")
    ...
    # Filter to only show core food categories for relevant days remaining calculation in the chart
    df_food_only = df[df['Is_Core_Food'] == True].copy()
    ...
    bar_chart = alt.Chart(df).mark_bar(opacity=0.8, color="#008CFF").encode(
        ...
        # Color based on days remaining (only food categories will show meaningful color scale)
        color=alt.Color('Days Remaining:Q', scale=alt.Scale(domain=[0, 30, 60, DAYS_IN_MISSION], range=['#FF4136', '#FFB703', '#A9D0F5', '#008CFF']), title="Est. Days Remaining (Food)"),
        # Tooltip for interaction
        tooltip=['Category', 'Current Stock', 'Original Stock', alt.Tooltip('Days Remaining', format='.0f', title="Est. Days Remaining")]
    ).properties(
    ...
    st.dataframe(df.set_index('Category').sort_values('Days Remaining', na_position='last'), 
                 ...
                 column_config={
                     ...
                     "Is_Core_Food": None # Hide the helper column
                 }) # Display raw data
...
def history_page():
    ...
    for entry in reversed(st.session_state.change_log):
        ...
        if action.startswith("RESTORATION"):
            ...
            if change_amount > 0 or action == 'none':
