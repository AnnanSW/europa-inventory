...
# --- INITIAL INVENTORY (UPDATED TO MEET 4 CREW / 10 MONTHS REQUIREMENT) ---
def get_initial_inventory():
    """
    Calculates initial stock based on 4 crew members for 10 months (9 mission + 1 redundancy).
    Target: ~3 meals + ~3 snacks + ~2 beverages/mixes + ~4 condiments = ~12 units/day/person.
    Total units needed: 12 * 4 * 304.4 = 14,611 units.
    
    The distribution is slightly adjusted for variety and realistic consumption patterns.
    """
    
    # Base daily units per crew (for 304.4 days)
    # The total target for all 50 items is 14,800 units.
    daily_unit_target = 12.15 # Approx daily units per person to hit 14,800 total
    
    # Calculate stock based on a multiplier relative to the base target
    # The sum of these multipliers is 1.0 (10 items per category * 0.1)
    
    base_stock_per_item = int(np.ceil(daily_unit_target * CREW_SIZE * DAYS_IN_MISSION / 50)) # ~296 units per item if distributed evenly
    
    def calculate_stock(multiplier):
        # Calculate the required stock for one item based on a multiplier of the base_stock_per_item
        # Units are calculated to be roughly 10 months worth of varied consumption.
        return int(np.ceil(base_stock_per_item * multiplier))

    # --- Hydratable Meals (High-volume/Main Dishes) ---
    stock_hm = [1.2, 0.9, 1.1, 1.0, 0.8, 1.5, 1.8, 0.9, 1.3, 1.0] # Multipliers for the 10 items
    
    # --- Thermostabilized Meals (Medium-volume/Entrees & Sides) ---
    stock_tm = [1.5, 1.0, 0.9, 1.2, 0.8, 1.4, 1.6, 0.7, 0.6, 1.3]
    
    # --- Natural Form & Irradiated (Snacks/Bread) ---
    ...
    # --- Condiments & Spreads (Very high-volume/Small packets) ---
    stock_cs = [3.5, 3.0, 5.0, 5.0, 1.5, 1.2, 0.8, 2.5, 4.0, 1.5]

    # Item names from the existing BARCODE_MAP (ordered)
    items_hm = [v[1] for k,v in BARCODE_MAP.items() if v[0] == "Hydratable Meals"]
    items_tm = [v[1] for k,v in BARCODE_MAP.items() if v[0] == "Thermostabilized Meals"]
    items_nf = [v[1] for k,v in BARCODE_MAP.items() if v[0] == "Natural Form & Irradiated"]
    items_db = [v[1] for k,v in BARCODE_MAP.items() if v[0] == "Desserts & Beverages (Non-Mix)"]
    items_cs = [v[1] for k,v in BARCODE_MAP.items() if v[0] == "Condiments & Spreads"]


    inventory_data = {
        "Hydratable Meals": {
            name: {"current": calculate_stock(mult), "original": calculate_stock(mult)}
    ...
    # Simulate some initial consumption (e.g., 2 months passed)
    days_passed = int(DAYS_IN_MISSION / 5) # Simulate 2 months consumption
    for category, items in inventory_data.items():
        for item_name, data in items.items():
            # A rough estimate of daily usage per item based on its multiplier
            # We divide the total original stock by the total mission days to get a daily consumption rate
            daily_consumption_rate = data['original'] / DAYS_IN_MISSION 
            consumed_units = int(np.floor(daily_consumption_rate * days_passed))
            data['current'] = max(0, data['original'] - consumed_units)
            
            # Apply some critical/low stock scenario manually for demonstration
            if "Chicken and Rice" in item_name:
                 data['current'] = 0 # Out of stock
            if "Tortillas" in item_name:
                 data['current'] = LOW_STOCK_THRESHOLD - 5 # Low stock
...
# --- DATA PREP FOR DASHBOARD (MODIFIED) ---
...
def dashboard_page():
    ...
    # --- Key Status Indicators (New Multi-Row Display) ---
    ...
    for i, category in enumerate(df['Category']):
        ...
        col_cat_title, col_units, col_days, col_status = st.columns([1.5, 1, 1, 1.5])
        ...
        with col_days:
            ...
            st.metric("Estimated Days Left", f"{int(days_rem):,}", delta=status_text, delta_color=delta_color)
    ...
    bar_chart = alt.Chart(df).mark_bar(opacity=0.8, color="#008CFF").encode(
        ...
        color=alt.Color('Days Remaining:Q', scale=alt.Scale(domain=[0, 30, 60, DAYS_IN_MISSION], range=['#FF4136', '#FFB703', '#A9D0F5', '#008CFF']), title="Days Remaining"),
        # Tooltip for interaction
        tooltip=['Category', 'Current Stock', 'Original Stock', alt.Tooltip('Days Remaining', format='.0f')]
    ).properties(
        title='Inventory Breakdown by Category (Days Remaining Highlight)'
    ).interactive()
    ...
    st.dataframe(df.set_index('Category').sort_values('Days Remaining'), 
...
def edit_item_page():
    ...
    with col1:
        add_units = st.number_input(
            ...
            # Max value is simply a large number for safety, but user can always override/refill
