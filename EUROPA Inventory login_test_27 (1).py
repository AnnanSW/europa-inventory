...
def record_change(category, item, old_amount, new_amount, action):
    ...
    """
    # First, apply the change to the inventory
    ...
    st.session_state.change_log.append({
        ...
        "allocations": copy.deepcopy(st.session_state.get("last_allocations_snapshot", {})),
        "category": category,
        "item": item,
        "action": action,
...
def crew_wide_stats_page():
    ...
    if st.session_state.current_user:
        ...
        st.markdown("<h3>Personal Inventory Responsibility Profile</h3>", unsafe_allow_html=True)

        user = st.session_state.current_user

        stats = get_user_consumption_stats(user)

        total_assigned = get_user_total_allocated(user)
        total_used = stats["total_removed"]

        if total_assigned == 0:
            st.warning("No allocation data found for this user yet.")
            return
        
        usage_pct = (total_used / total_assigned * 100) if total_assigned > 0 else 0

        col1, col2, col3 = st.columns(3)

        col1.metric("Assigned Units", f"{total_assigned:,}")
        col2.metric("Units Used", f"{total_used:,}")
        col3.metric("Usage %", f"{usage_pct:.1f}%")

        st.markdown("#### Allocation Breakdown")

        for crew, amt in allocations.items():
            st.write(f"• {crew}: {amt} units assigned")

    else:
        st.info("Log in to view personal allocation data.")
...
def get_user_consumption_stats(username):
    """
    Extracts consumption data for a specific crew member from change_log.
    """
    ...
    for entry in user_logs:
        ...
        if action == "remove":
            diff = entry["old_amount"] - entry["new_amount"]
            total_removed += diff

        elif action == "add":
            diff = entry["new_amount"] - entry["old_amount"]
            total_added += diff
...
def create_user_inventory_dataframe(username):
    """
    Creates a filtered view of inventory based on what a user is responsible for
    using latest allocation snapshots in change_log.
    """

    df, _, _, _, _ = create_inventory_dataframe()

    # get latest allocation snapshot for user
    allocations = get_user_allocation_profile(username)

    if not allocations:
        return df, 0, 0, 0, 0

    total_assigned_ratio = sum(allocations.values())

    # scale inventory down proportionally
    user_rows = []

    for _, row in df.iterrows():
        category = row["Category"]

        # proportional share of category based on total allocation ratio
        share_factor = allocations.get(username, 0) / total_assigned_ratio if total_assigned_ratio else 0

        user_current = row["Current Stock"] * share_factor
        user_original = row["Original Stock"] * share_factor

        user_rows.append({
            "Category": category,
            "Current Stock": user_current,
            "Original Stock": user_original,
            "Consumed Stock": user_original - user_current,
            "Days Remaining": row["Days Remaining"] * share_factor if row["Days Remaining"] != np.inf else np.inf,
            "Consumption Rate": row["Consumption Rate"]
        })

    user_df = pd.DataFrame(user_rows)

    total_current = user_df["Current Stock"].sum()
    total_original = user_df["Original Stock"].sum()

    low_stock_count = 0
    zero_stock_count = 0

    return user_df, total_current, total_original, low_stock_count, zero_stock_count

def get_user_item_breakdown(username):
    """
    Builds per-item consumption totals for a specific user
    based on allocation splits in change_log.
    """
    ...
    for entry in logs:
        ...
        # allocations already contains per-user split
        user_amount = allocations.get(username, 0)

        key = f"{category} > {item}"
        item_totals[key] = item_totals.get(key, 0) + user_amount

    return item_totals

def get_user_total_allocated(username):
    total = 0

    for entry in st.session_state.change_log:
        alloc = entry.get("allocations", {})
        if username in alloc:
            total += alloc[username]

    return total

def get_user_allocation_profile(username):
    """
    Reconstructs approximate allocation responsibility for a user
    using change_log snapshots.
    """
    ...
    # find most recent allocation snapshot for user
    for entry in reversed(logs):
        alloc = entry.get("allocations", {})
        if username in alloc:
            latest_allocation = alloc
            break
...
def dashboard_page():
    ...
    if st.session_state.current_user:
        df, total_current, total_original, low_stock_count, zero_stock_count = create_user_inventory_dataframe(
    ...
    # --- PERSONAL USER STATS (ONLY WHEN LOGGED IN) ---
    if st.session_state.current_user:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<h3>Personal Crew Consumption Profile</h3>", unsafe_allow_html=True)

        user = st.session_state.current_user

        stats = get_user_consumption_stats(user)
        item_breakdown = get_user_item_breakdown(user)
        ...
        st.markdown("#### Personal Item Usage Breakdown")
        ...
        if item_breakdown:
            ...
            for item, amt in sorted_items:
                st.write(f"• {item}: {amt} units consumed")
        else:
            st.info("No per-item usage recorded yet.")
...
def edit_item_page():
    ...
    if st.session_state.change_applied:
        ...
        if new_amount < 0:
            st.error("❌ **ERROR:** Cannot go below zero.")
            return

        elif remove_val > 0:
            if not allocations:
                st.error("❌ **ERROR:** You must assign items to at least one crew member.")
                return
            elif total_allocated != remove_val:
                st.error(f"❌ **ERROR:** Removed units ({remove_val}) must equal total assigned ({total_allocated}).")
                return
            else:
                action = "remove"
        ...
        # ✅ APPLY CHANGE (only runs if valid)
        st.session_state.last_allocations_snapshot = crew_allocations

        record_change(category, item, old_amount, new_amount, action)

        # If we added units, update the 'original' capacity
