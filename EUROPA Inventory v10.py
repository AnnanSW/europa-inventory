...
# --- INITIAL INVENTORY (UPDATED TO START AT MAX CAPACITY) ---
def get_initial_inventory():
    """
    Calculates initial stock based on 4 crew members for 10 months (9 mission + 1 redundancy).
    All items start with current stock equal to original stock (max capacity).
    """
    ...
    # --- Multipliers for the 10 items in each category ---
    ...
    # --- REMOVED SIMULATED CONSUMPTION ---
    # The 'current' stock is now equal to 'original' stock for all items.
    
    # Optional: Apply some critical/low stock scenario manually for demonstration *if needed*
    # For a fully stocked start, we will remove this for now.
...
def dashboard_page():
    ...
    for i, category in enumerate(df['Category']):
        ...
        # Since we start fully stocked, Days Remaining should be ~304.4 for all.
        # We simulate status based on how far we are from that max for initial view.
        if days_rem <= 30:
            delta_color = "inverse"
