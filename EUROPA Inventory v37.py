...
# --- MISSION PARAMETERS (Editable via Settings) ---
if "crew_size" not in st.session_state:
    st.session_state.crew_size = 7
if "mission_days" not in st.session_state:
    st.session_state.mission_days = 1035
if "redundancy_factor" not in st.session_state:
    st.session_state.redundancy_factor = 1.30

CREW_SIZE = st.session_state.crew_size
MISSION_DAYS = st.session_state.mission_days
REDUNDANCY_FACTOR = st.session_state.redundancy_factor
...
# --- INITIAL INVENTORY GENERATOR (Fixed to use current session_state mission parameters) ---
def get_initial_inventory():
    """
    Generates the initial inventory for all items based on current mission parameters:
    Crew size, mission days, and redundancy factor.
    """

    # --- Pull current mission parameters ---
    crew_size = st.session_state.crew_size
    mission_days = st.session_state.mission_days
    redundancy_factor = st.session_state.redundancy_factor
    days_in_mission = int(np.ceil(mission_days * redundancy_factor))
    ...
    # --- Meal Constants ---
    MEALS_PER_PERSON_PER_DAY = 3  # Standard NASA-sized portion
    MEAL_WEIGHTS = {
    ...
    # --- Category multipliers (simulated stock variation) ---
    ...
    # --- Gather items per category from BARCODE_MAP ---
    category_items = {
    ...
    # --- Total meals required for the mission ---
    total_meals_required = int(np.ceil(crew_size * MEALS_PER_PERSON_PER_DAY * days_in_mission))

    # --- Compute total weighted units ---
    ...
    # Assign uniform stock per category based on multipliers
    ...
    # --- Condiments & Spreads ---
    # Scale roughly 1 per entrée pouch
    total_entrees = sum(
        item["original"]
...
def revert_version(version_id):
    try:
        entry = st.session_state.change_log[version_id - 1]
        # Deep copy of the inventory *before restoring* for logging
        inventory_before_restore = copy.deepcopy(st.session_state.inventory)

        # Restore inventory
        restored_state = copy.deepcopy(entry["state_before_change"])
        st.session_state.inventory = restored_state

        # Restore mission parameters if applicable
        if entry["category"] == "SYSTEM" and entry["item"] == "MISSION_PARAMETERS":
            old_vals = entry["old_amount"]
            st.session_state.crew_size = old_vals["crew_size"]
            st.session_state.mission_days = old_vals["mission_days"]
            st.session_state.redundancy_factor = old_vals["redundancy_factor"]

        # Log the restoration (use the inventory *before restore* to avoid overwriting)
        st.session_state.change_log.append({
            "version_id": len(st.session_state.change_log) + 1,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "category": "SYSTEM",
            ...
            "old_amount": "N/A",
            "new_amount": "N/A",
            "state_before_change": inventory_before_restore
        })

    except Exception as e:
        st.error(f"Invalid version ID or restore failed: {e}")
...
def create_inventory_dataframe():
    ...
    for category, items in st.session_state.inventory.items():
        ...
        # Days remaining calculation based on updated mission days and redundancy
...
def inventory_page():
    col_settings, col_title = st.columns([1, 10])

    with col_settings:
        if st.button("⚙", help="System Settings"):
...
def settings_page():
    ...
    crew = st.number_input(
        ...
        min_value=1,
        max_value=20,
        value=st.session_state.crew_size
    )
    ...
    days = st.number_input(
        ...
        min_value=1,
        max_value=5000,
        value=st.session_state.mission_days
    )
    ...
    redundancy = st.number_input(
        ...
        max_value=2.0,
        step=0.05,
        value=st.session_state.redundancy_factor
    )

    st.markdown("<hr>", unsafe_allow_html=True)

    if st.session_state.change_applied:
        st.success("✅ Mission parameters updated. Inventory recalculated.")
        st.button("🚀 Return to Inventory Home", on_click=lambda: setattr(st.session_state, "change_applied", False) or go_to_inventory())
    elif st.button("✅ Apply & Recalculate Inventory"):
        # Record previous state for version history
        old_params = {
            "crew_size": st.session_state.crew_size,
            "mission_days": st.session_state.mission_days,
            "redundancy_factor": st.session_state.redundancy_factor
        ...
        st.session_state.crew_size = int(crew)
        st.session_state.mission_days = int(days)
        st.session_state.redundancy_factor = float(redundancy)
        ...
        st.session_state.change_log.append({
            ...
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "category": "SYSTEM",
            "item": "MISSION_PARAMETERS",
            "action": "UPDATE",
            "old_amount": old_params,
            "new_amount": {
                "crew_size": st.session_state.crew_size,
                "mission_days": st.session_state.mission_days,
                "redundancy_factor": st.session_state.redundancy_factor
...
def history_page():
    ...
    for entry in reversed(st.session_state.change_log):
        ...
        elif category == "SYSTEM" and item == "MISSION_PARAMETERS":
            # Display mission parameter changes neatly
            old_vals = old_amount
            new_vals = new_amount
            st.markdown(
                f"**V{version_id}** | {timestamp} | MISSION PARAMETERS UPDATED:\n"
                f"- Crew Size: {old_vals['crew_size']} → {new_vals['crew_size']}\n"
                f"- Mission Days: {old_vals['mission_days']} → {new_vals['mission_days']}\n"
                f"- Redundancy Factor: {old_vals['redundancy_factor']:.2f} → {new_vals['redundancy_factor']:.2f}",
                unsafe_allow_html=True
            )
        else:
        ...
        # Add restore button for any version
