...
def get_predicted_daily_consumption():
    """
    Returns predicted daily consumption for the entire crew per category (without redundancy).
    """
    daily_consumption = {
        "Hydratable Meals": CREW_SIZE * 3,  # Meals (Hydratable + Thermostabilized)
        "Thermostabilized Meals": CREW_SIZE * 3,  # Split the total meals equally
        "Natural Form & Irradiated": int(CREW_SIZE * 1.5),
        "Desserts & Beverages (Non-Mix)": CREW_SIZE * 10,
        "Condiments & Spreads": None  # Specific to Entree/Snack/Dessert; can be computed if needed
    }
...
def create_inventory_dataframe():
    ...
    # Use precise daily consumption numbers per category
    daily_consumption = get_predicted_daily_consumption()
    ...
    for category, items in st.session_state.inventory.items():
        ...
        # Compute Days Remaining using predicted daily consumption
        pred_daily = daily_consumption.get(category, 1)  # fallback to 1 if undefined
        days_remaining = np.floor(cat_current / pred_daily) if pred_daily > 0 else np.inf
...
def inventory_page():
    ...
    # --- Current Mission Day Indicator (Inventory page, nicer look) ---
    all_edits = st.session_state.change_log
    if all_edits:
        latest_timestamp = max(
            datetime.strptime(entry["timestamp"], "%Y-%m-%d %H:%M:%S") for entry in all_edits
        )
        first_timestamp = datetime.strptime(all_edits[0]["timestamp"], "%Y-%m-%d %H:%M:%S")
        current_day = min(MISSION_DAYS, (latest_timestamp - first_timestamp).days + 1)
    else:
        current_day = 1

    days_left = MISSION_DAYS - current_day
    st.markdown(f"**Mission Day:** {current_day} / {MISSION_DAYS} | Days Left: {days_left}")
    
    # Mini progress bar for homepage
    progress_ratio = current_day / MISSION_DAYS
...
def dashboard_page():
    ...
    # --- Prepare Inventory Data with Precise Predicted Consumption ---
    df, total_current, total_original, low_stock_count, zero_stock_count = create_inventory_dataframe()

    # --- Compute Predicted Daily Consumption Per Category ---
    # Entire crew, no redundancy
    crew_daily = {
        "Hydratable Meals": CREW_SIZE * 3,
        "Thermostabilized Meals": CREW_SIZE * 3,
        "Natural Form & Irradiated": CREW_SIZE * 1.5,
        "Desserts & Beverages (Non-Mix)": CREW_SIZE * 10,
        "Condiments & Spreads": None  # Condiments handled differently
    }

    # Days remaining per category using predicted daily consumption
    days_remaining_precise = {}
    for category, items in st.session_state.inventory.items():
        if category in ["Hydratable Meals", "Thermostabilized Meals"]:
            daily_use = crew_daily["Hydratable Meals"] if category == "Hydratable Meals" else crew_daily["Thermostabilized Meals"]
        elif category in ["Natural Form & Irradiated"]:
            daily_use = crew_daily[category]
        elif category in ["Desserts & Beverages (Non-Mix)"]:
            daily_use = crew_daily[category]
        else:
            daily_use = 0  # Condiments excluded
        total_units = sum(item['current'] for item in items.values())
        days_remaining_precise[category] = np.floor(total_units / daily_use) if daily_use > 0 else np.inf

    # --- Global Metrics Section ---
    st.markdown("<h3>Mission-Wide Status</h3>", unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)

    total_days_left_precise = min(days_remaining_precise.values())
    ...
    col4.metric("Total Days Left (All Crew, Precise)", f"{int(total_days_left_precise):,} days")
    ...
    for _, data in df.iterrows():
        ...
        days_rem = days_remaining_precise.get(category, np.inf)
        ...
        col_days.metric("Days Left (All Crew, Precise)", f"{int(days_rem):,}", delta=status_text, delta_color=delta_color)
