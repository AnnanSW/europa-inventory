...
def get_initial_inventory():
    ...
    - Condiments excluded from caloric intake
    """
    ...
    total_meals_required = int(
        np.ceil(CREW_SIZE * MEALS_PER_PERSON_PER_DAY * MISSION_DAYS * REDUNDANCY_FACTOR)
    )
    ...
    # Category multipliers (UNCHANGED)
    ...
    for category, (items, multipliers) in category_items.items():
        ...
        for name, mult in zip(items, multipliers):
            units = int(np.ceil(base_units_per_weight * meal_weight * mult))
            inventory_data[category][name] = {
                "current": units,
                "original": units
            }

    # --- CONDIMENTS (NON-CALORIC SUPPORT) ---
    total_entrees = sum(
        item["original"]
