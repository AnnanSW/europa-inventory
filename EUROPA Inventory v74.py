...
MISSION_START_DATE = datetime(2031, 6, 1) # Year, Month, Day

# --- INVENTORY ALERTS ---
LOW_STOCK_PERCENT = 0.05
...
def get_item_color(current, original):
    if current == 0:
        return "#FF4136"  # Red
    if original > 0 and (current / original) <= LOW_STOCK_PERCENT:
        return "#FFB703"  # Amber (≤5%)
    return "#F0F0F0"
...
def get_category_color(category):
    ...
    for item in st.session_state.inventory[category].values():
        ...
        # Fixed: use LOW_STOCK_PERCENT instead of undefined LOW_STOCK_THRESHOLD
        if item["original"] > 0 and (item["current"] / item["original"]) <= LOW_STOCK_PERCENT:
...
def revert_version(version_id):
    try:
        ...
        # Keep a snapshot BEFORE restore
        pre_restore_state = copy.deepcopy(st.session_state.inventory)
        ...
        st.session_state.change_log.append({
            ...
            "old_amount": "N/A",
            "new_amount": "N/A",
            "state_before_change": pre_restore_state
        })
...
def inventory_page():
    ...
    current_day = (datetime.now() - MISSION_START_DATE).days + 1
    current_day = min(current_day, MISSION_DAYS)
...
def dashboard_page():
    ...
    bar_chart = alt.Chart(df).mark_bar(opacity=0.8).encode(
...
def category_page():
    ...
    for item_name, data in items.items():
        color = get_item_color(data["current"], data["original"])
        ...
        elif data["original"] > 0 and (data["current"] / data["original"]) <= LOW_STOCK_PERCENT:
            status_emoji = "⚠️"
            status_text = "LOW STOCK (<5%)"
        else:
            status_emoji = "✅"
            status_text = "NOMINAL"
...
def edit_item_page():
    ...
    # Back button only if changes not applied
    if not st.session_state.change_applied:
        st.button("⬅ Back to Category", on_click=lambda: go_to_category(category))

    # Current and original units display
    st.markdown(f"**Current Units:** {data['current']}  |  **Original Load:** {data['original']}")

    # Progress bar based on current/original
    if data["original"] > 0:
        st.progress(data["current"] / data["original"])
    else:
        st.progress(0)

    # --- LOW STOCK WARNING (fixed: use LOW_STOCK_PERCENT) ---
    if data["original"] > 0 and (data["current"] / data["original"]) <= LOW_STOCK_PERCENT:
        st.warning(
            f"⚠️ **LOW STOCK WARNING!** Less than **5%** remaining "
            f"({data['current']} / {data['original']} units)."
        )
    ...
    with col1:
        add_units = st.number_input(
            "Add Units (Resupply)",
            min_value=0,
            max_value=data["original"] * 2,  # Allow resupply beyond original
    ...
    if st.session_state.change_applied:
        ...
        if new_amount < 0:
            ...
            if action != "none":
                ...
                # If resupply exceeds original capacity, update original max
...
def consumption_history_page():
    ...
    # --- FIXED: Reference all edits from session state
