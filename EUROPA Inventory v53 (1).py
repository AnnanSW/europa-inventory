...
def history_page():
    ...
    # --- Predicted Units Banner (per crew, permanent) ---
    predicted_daily = {
        "Hydratable Meals": CREW_SIZE * 3,
        "Thermostabilized Meals": CREW_SIZE * 3,
        "Natural Form & Irradiated": CREW_SIZE * 1.5,
    ...
    st.info(
        f"""
**Predicted Daily Usage for Entire Crew**
- Hydratable Meals: {predicted_daily['Hydratable Meals']} units
- Thermostabilized Meals: {predicted_daily['Thermostabilized Meals']} units
- Natural Form & Irradiated: {predicted_daily['Natural Form & Irradiated']} units
- Desserts & Beverages: {predicted_daily['Desserts & Beverages (Non-Mix)']} units
- Condiments & Spreads: {predicted_daily['Condiments & Spreads']}
""",
        icon="ℹ️"
    )
    ...
    # --- Start / End day filters, default 1–7 ---
    col_start, col_end = st.columns(2)
    with col_start:
        start_day = st.number_input("Start Day", min_value=1, max_value=DAYS_IN_MISSION, value=1)
    with col_end:
        end_day = st.number_input("End Day", min_value=1, max_value=DAYS_IN_MISSION, value=7)
    ...
    sorted_days = sorted(day_groups.keys(), reverse=True)  # newest day first
    ...
    for day in sorted_days:
        ...
        daily_usage = {
            "Meals": 0,  # Hydratable + Thermostabilized
            "Natural Form & Irradiated": 0,
            "Desserts & Beverages (Non-Mix)": 0,
            "Condiments & Spreads": "Specific to Entree, Snack, or Dessert"
        }
        ...
        for entry in day_groups[day]:
            ...
            if isinstance(old_amount, int) and isinstance(new_amount, int):
                consumed = old_amount - new_amount
                if category in ["Hydratable Meals", "Thermostabilized Meals"]:
                    daily_usage["Meals"] += consumed
                elif category == "Natural Form & Irradiated":
                    daily_usage["Natural Form & Irradiated"] += consumed
                elif category == "Desserts & Beverages (Non-Mix)":
                    daily_usage["Desserts & Beverages (Non-Mix)"] += consumed

        # --- Show banner for this day ---
        st.info(
            f"**Daily Consumption Totals**\n"
            f"- Meals (Hydratable + Thermostabilized): {daily_usage['Meals']}\n"
            f"- Natural Form & Irradiated: {daily_usage['Natural Form & Irradiated']}\n"
            f"- Desserts & Beverages: {daily_usage['Desserts & Beverages (Non-Mix)']}\n"
            f"- Condiments & Spreads: {daily_usage['Condiments & Spreads']}"
        )

        # --- Display versions for this day, newest first ---
        for entry in sorted(day_groups[day], key=lambda x: x["version_id"], reverse=True):
            ...
            if st.button(display_text, key=f"restore_{version_id}"):
                ...
                st.session_state.restore_trigger = True  # Set flag instead of rerun
                st.success(f"Inventory restored to state at V{version_id}")

            # At the very end of history_page() function
            if st.session_state.restore_trigger:
                st.session_state.restore_trigger = False
                st.experimental_rerun()
