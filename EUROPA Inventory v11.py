...
# --- BARCODE MAPPING (Updated with 13-digit codes) ---
...
def get_initial_inventory():
    ...
    Core food (4 categories) stock is calculated based on CREW_SIZE and DAYS_IN_MISSION.
    Condiments stock is set high and does not influence core calculations.
    """
    
    # 1. CORE FOOD CALCULATION (Hydratable, Thermostabilized, Natural, Desserts)
    # Base daily units per crew (for 304.4 days) - Total units needed divided by 4 core categories
    # Assuming ~14,800 total units are needed for core food
    target_units_per_category = 14800 / 4 # Total required food units
    # Calculate the base stock per item for the 4 core categories (10 items each, 40 total)
    ...
    # --- Multipliers for the 4 core categories ---
    ...
    # Item names from the existing BARCODE_MAP (ordered)
    items_hm = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101890")] # Adjusted logic for category finding
    items_tm = [v[1] for k,v in BARCODE_MAP.items() if v[0].startswith("93130101891") and not v[0].startswith("931301018910")] # Fix for 931301018910
    ...
    # 2. CONDIMENTS STOCK (Set high, outside of core caloric calc)
    # Condiment base stock is set much higher to reflect a non-caloric/high-supply requirement
    # Base is set to ensure a substantial starting inventory for condiments
    base_stock_condiments = 2000 
    
    def calculate_condiment_stock(multiplier):
        # Calculate the required stock for one CONDIMENT item
        return int(np.ceil(base_stock_condiments * multiplier / 50)) 

    # Condiment multipliers (increased to ensure high stock)
...
# --- DATA PREP FOR DASHBOARD (Updated to exclude Condiments from total food calculation) ---
def create_inventory_dataframe():
    ...
    # List of categories considered 'core food' for mission life calculations
...
def dashboard_page():
    ...
    # ADJUSTED COLUMNS FOR MORE TITLE SPACE (1.8, 1, 1, 1.2)
    for i, category in enumerate(df['Category']):
        ...
        # ADJUSTED THE COLUMN RATIOS TO FIX THE ELLIPSES ISSUE
