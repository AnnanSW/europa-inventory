...
def inventory_page():
    ...
    # 2. NAVIGATION BUTTONS (updated for Consumption History)
    col_nav1, col_nav2 = st.columns([1, 1])
    with col_nav1:
        if st.button("📊 Inventory Dashboard", help="View visual stock summary", use_container_width=True):
            go_to_dashboard()
    with col_nav2:
        if st.button("📚 Consumption History", help="Review and revert changes", use_container_width=True):
            st.session_state.page = "History"  # Matches the key in main switch
...
def consumption_history_page():
    ...
    total_days = DAYS_IN_MISSION
    # Calculate consumed days based on edits
    all_edits = st.session_state.change_log
    # For simplicity, let's assume the number of unique edit days is the "current mission day"
    if all_edits:
        latest_timestamp = max(
            datetime.strptime(entry["timestamp"], "%Y-%m-%d %H:%M:%S") for entry in all_edits
        )
        current_day = min(total_days, (latest_timestamp - datetime.strptime(all_edits[0]["timestamp"], "%Y-%m-%d %H:%M:%S")).days + 1)
    else:
        current_day = 1
    
    st.markdown(f"**Current Mission Day:** {current_day} / {total_days}")
    st.progress(current_day / total_days)
    ...
    with col_start:
        start_day = st.number_input("Start Mission Day", min_value=1, max_value=total_days, value=default_start, step=1)
    with col_end:
        end_day = st.number_input("End Mission Day", min_value=1, max_value=total_days, value=default_end, step=1)
...
""", icon="ℹ️")
    ...
    # Convert timestamps to mission day number relative to first edit
    def get_mission_day(ts_str):
        ts = datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S")
        first_ts = datetime.strptime(all_edits[0]["timestamp"], "%Y-%m-%d %H:%M:%S")
        day_num = (ts - first_ts).days + 1
        return day_num

    # Build a dictionary: day_num -> list of edits
    day_edits = {}
    ...
    # Display edits day by day, most recent first
    for day_num in sorted(day_edits.keys(), reverse=True):
        ...
        for entry in reversed(day_edits[day_num]):  # Most recent edits on top
            ...
            # Compose the button label with info
            ...
            # The button itself restores to that version
