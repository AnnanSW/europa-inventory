...
def get_personal_inventory_view(username):
    ...
    # --- STEP 1: Find latest restore (if any) ---
    latest_restore_index = -1
    for i, entry in enumerate(logs):
        if "RESTORE" in entry["action"]:
            latest_restore_index = i

    # Only consider logs AFTER the last restore
    valid_logs = logs[latest_restore_index + 1:]
