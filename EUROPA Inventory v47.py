...
# --- CONSUMPTION RATES / CHANGE LOG PAGE ---
def history_page():
    st.markdown("<h1>📊 Consumption Rates / Change Log</h1>", unsafe_allow_html=True)
    st.button("⬅ Back to Inventory", on_click=go_to_inventory)
    st.markdown("<hr>", unsafe_allow_html=True)

    # --- Predicted Consumption Info Button ---
    if st.button("ℹ️ Predicted Units per Day (by Category)"):
        predicted_per_day = {}
        MEALS_PER_PERSON_PER_DAY = 3
        CREW_SNACK_MEAL = 0.25  # estimated snack/dessert per person per day
        for category, items in st.session_state.inventory.items():
            predicted_units = 0
            if category in ["Hydratable Meals", "Thermostabilized Meals"]:
                predicted_units = len(items) * MEALS_PER_PERSON_PER_DAY * CREW_SIZE
            elif category in ["Natural Form & Irradiated", "Desserts & Beverages (Non-Mix)"]:
                predicted_units = len(items) * CREW_SIZE * CREW_SNACK_MEAL
            elif category == "Condiments & Spreads":
                # proportional to main meals
                total_entrees = sum(
                    st.session_state.inventory[cat][name]["original"]
                    for cat in ["Hydratable Meals", "Thermostabilized Meals"]
                    for name in st.session_state.inventory[cat]
                )
                predicted_units = int(total_entrees / len(items))
            predicted_per_day[category] = int(predicted_units)
        st.write(predicted_per_day)

    # --- Determine current mission day and version ---
    ...
    # --- Display days (most recent first) ---
    for day in sorted_days:
        day_total_used = defaultdict(int)
        predicted_day_total = {}
        # Recalculate predicted for the day (crew × meals/snacks etc)
        for category, items in st.session_state.inventory.items():
            if category in ["Hydratable Meals", "Thermostabilized Meals"]:
                predicted_day_total[category] = len(items) * CREW_SIZE * 3
            elif category in ["Natural Form & Irradiated", "Desserts & Beverages (Non-Mix)"]:
                predicted_day_total[category] = int(len(items) * CREW_SIZE * 0.25)
            elif category == "Condiments & Spreads":
                total_entrees = sum(
                    st.session_state.inventory[cat][name]["original"]
                    for cat in ["Hydratable Meals", "Thermostabilized Meals"]
                ...
                predicted_day_total[category] = int(total_entrees / len(items))

        # Compute actual consumption for this day
        for entry in day_groups[day]:
            if isinstance(entry['new_amount'], (int, float)) and isinstance(entry['old_amount'], (int, float)):
                cat = entry['category']
                used_units = entry['old_amount'] - entry['new_amount']
                day_total_used[cat] += used_units

        # --- Determine flag based on +/- tolerance ---
        day_flag_text = ""
        over_categories = []
        under_categories = []

        for cat, predicted_units in predicted_day_total.items():
            actual_used = day_total_used.get(cat, 0)
            if actual_used > predicted_units * 1.02:
                over_categories.append(f"{cat} (+{actual_used - predicted_units})")
            elif actual_used < predicted_units * 0.85:
                under_categories.append(f"{cat} (-{predicted_units - actual_used})")

        if over_categories:
            day_flag_text = f"❌ Ration Exceeded: {', '.join(over_categories)}"
        elif under_categories:
            day_flag_text = f"⚠️ Ration Underused: {', '.join(under_categories)}"
        else:
            day_flag_text = "✅ Predicted amount used"

        st.markdown(f"<h3>Mission Day {day} | {len(day_groups[day])} changes | {day_flag_text}</h3>", unsafe_allow_html=True)

        # --- Version entries for the day ---
        for entry in day_groups[day]:
            ...
            if action.startswith("RESTORATION_TO"):
                display_text = f"V{version_id} | {timestamp} | {action}"
            else:
                display_text = f"V{version_id} | {timestamp} | {category} > {item} | {change_amount:+}"
