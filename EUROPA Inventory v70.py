...
def inventory_page():
    ...
    with st.form("barcode_scan_form", clear_on_submit=True):
        scanned_barcode = st.text_input(
            "SCAN BARCODE",
            key="barcode_input",
            help="Scan a product barcode here"
        )
    ...
    # --- AUTO-FOCUS BARCODE INPUT (JS WORKAROUND) ---
    st.components.v1.html(
        """
        <script>
        const inputs = window.parent.document.querySelectorAll('input');
        for (let i = 0; i < inputs.length; i++) {
            if (inputs[i].getAttribute('aria-label') === 'SCAN BARCODE') {
                inputs[i].focus();
                break;
            }
        }
    ...
    with col_nav2:
        if st.button("📚 Consumption History", use_container_width=True):
    ...
    for i, category in enumerate(st.session_state.inventory.keys()):
        ...
        with cols[i % 3]:
            if st.button(button_label, key=f"catbtn_{category}"):
