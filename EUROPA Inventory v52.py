    ...
    for category, items in st.session_state.inventory.items():
        ...
        data_list.append({
            ...
            category_daily_consumption = {
                ...
                "Condiments & Spreads": None  # handled elsewhere
            }

            days_left = np.inf
            if category in ["Hydratable Meals", "Thermostabilized Meals", "Natural Form & Irradiated", "Desserts & Beverages (Non-Mix)"]:
                daily_total_units = category_daily_consumption[category] * REDUNDANCY_FACTOR
                days_left = np.floor(cat_current / daily_total_units) if daily_total_units > 0 else np.inf
