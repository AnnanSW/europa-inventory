...
# --- HISTORY / CONSUMPTION RATES PAGE (Fixed) ---
def history_page():
    ...
    if st.button("⬅ Back to Inventory"):
        go_to_inventory()
        return  # immediately exit function to avoid partial reruns / double clicks
    ...
    # Filter change log entries by day
    ...
    # --- Display each day and its versions ---
    for day in sorted_days:
        entries = day_groups[day]
        ...
        # --- Display each version in reverse (newest first) ---
        for entry in sorted(entries, key=lambda x: x["version_id"], reverse=True):
            ...
            # Show numeric change if possible
            if isinstance(old_amount, int) and isinstance(new_amount, int):
                change_str = f"{old_amount} → {new_amount}"
            else:
                change_str = ""

            # Label for restored versions
            if category == "SYSTEM" and item == "RESTORE":
                restored_to = entry.get("action", "").replace("RESTORATION_TO_V", "")
                action_label = f"RESTORED → V{restored_to}"
            elif category == "SYSTEM" and item == "INITIAL_LOAD":
                action_label = "INITIAL LOAD"
            else:
            ...
            if st.button(display_text, key=f"restore_{version_id}"):
                ...
                return  # exit immediately so the UI refreshes correctly
