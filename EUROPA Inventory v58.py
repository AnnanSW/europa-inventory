...
def history_page():
    ...
    # --- Handle page restore trigger ---
    if st.session_state.get("restore_triggered", False):
        st.session_state.restore_triggered = False
        return  # Stop current run and rerun cleanly
    ...
    # --- Filter change log by day range ---
    ...
    for day in sorted_days:
        ...
        st.markdown(f"<h3>Mission Day {day} | {len(entries)} change(s)</h3>", unsafe_allow_html=True)
        st.markdown("<hr>", unsafe_allow_html=True)
        ...
        for entry in sorted(entries, key=lambda x: x["version_id"], reverse=True):
            ...
            # Show numeric change as arrow if both numbers are int
            ...
            if category == "SYSTEM" and item != "INITIAL_LOAD":
                action_label = f"RESTORED TO V{version_id}"
            else:
            ...
            if st.button(display_text, key=f"restore_{version_id}"):
                ...
                # Record new version for restoration
                st.session_state.change_log.append({
                    "version_id": len(st.session_state.change_log) + 1,
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "category": "SYSTEM",
