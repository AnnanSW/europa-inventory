...
def revert_version(version_id):
    try:
        ...
        st.session_state.change_log.append({
            ...
            "old_amount": "N/A",
            "new_amount": "N/A",
            "state_before_change": copy.deepcopy(old_state),
            "state_after_change": copy.deepcopy(st.session_state.inventory),
            "restored_from": version_id
        })

        # 🔥 CRITICAL FIX FOR STREAMLIT STATE CONSISTENCY
        st.session_state.add_units_edit = 0
        st.session_state.remove_units_edit = 0
...
def get_personal_inventory_view(username):
    ...
    for entry in st.session_state.change_log:
        ...
        if head is not None and entry["version_id"] > head:
            continue

        # 🚨 Step 3 (important addition)
        if "RESTORE" in entry["action"]:
        continue
