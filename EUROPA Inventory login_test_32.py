...
def revert_version(version_id):
    try:
        ...
        # force all derived UI to refresh cleanly
        st.session_state.change_applied = False
        st.session_state.add_units_edit = 0
        st.session_state.remove_units_edit = 0
        st.session_state.barcode_error = None
    ...
    for category, items in inventory.items():
        ...
        for entry in logs:
            ...
            if category_entry == category:
                category_used += allocations.get(username, 0)
...
def dashboard_page():
    ...
    if is_crew_view():
        ...
        st.markdown("<h3>Key Status Indicators (Personal View)</h3>", unsafe_allow_html=True)
