...
def get_personal_inventory_view(username):
    """
    Returns a personal view of inventory that:
    - Responds to restores (uses current state)
    - Accounts for resupply (added units)
    - Subtracts ONLY what was consumed FOR the user
    """
    ...
    for category, items in inventory.items():
        cat_current_total = sum(i["current"] for i in items.values())
        cat_original_total = sum(i["original"] for i in items.values())

        # Step 1: split ORIGINAL evenly
        personal_original = cat_original_total / CREW_SIZE

        # Step 2: compute how much was consumed FOR this user
        used_by_user = 0
        ...
        for entry in logs:
            ...
            used_by_user += allocations.get(username, 0)

        # Step 3: derive CURRENT personal share from SYSTEM STATE
        # (this is the key fix — uses real inventory after restores/adds)

        personal_current = personal_original - used_by_user

        # clamp safety
        if personal_current < 0:
            personal_current = 0

        personal_data.append({
            "Category": category,
            "Current Stock": personal_current,
            "Original Stock": personal_original,
            "Consumed Stock": personal_original - personal_current,
            "Days Remaining": np.floor(
                personal_current / (personal_original / DAYS_IN_MISSION)
            ) if personal_original > 0 else 0,
            "Consumption Rate": (personal_original - personal_current) / personal_original if personal_original > 0 else 0
        })

        total_current += personal_current
        total_original += personal_original

    df = pd.DataFrame(personal_data)

    return df, total_current, total_original
...
def dashboard_page():
    ...
    if st.session_state.current_user:
        df, total_current, total_original = get_personal_inventory_view(st.session_state.current_user)

        # Recompute alerts for personal view
        low_stock_count = sum(1 for _, row in df.iterrows() if row["Current Stock"] <= LOW_STOCK_THRESHOLD and row["Current Stock"] > 0)
        zero_stock_count = sum(1 for _, row in df.iterrows() if row["Current Stock"] == 0)
    ...
    if st.session_state.current_user:
        ...
        pdata, p_current, p_original = get_personal_inventory_view(user)
        p_used = p_original - p_current
        ...
        if p_original > 0:
            ...
            if personal_daily_rate > 0:
                personal_days_left = np.floor(p_current / personal_daily_rate)
            else:
        ...
        for _, row in pdata.iterrows():
            ...
            remaining = row.get("Current Stock", 0)
            original = row.get("Original Stock", 0)
    ...
    if st.session_state.current_user:
        st.markdown("<h3>Key Status Indicators (Personal)</h3>", unsafe_allow_html=True)
    else:
...
def consumption_history_page():
    ...
    if st.session_state.current_user:
        st.info(f"""
    **Predicted Consumption Per Day (Individual Crew Member)**

    - Meals (Hydratable + Thermostabilized): 3  
    - Natural Form + Irradiated: 1.5  
    - Desserts + Beverages: 10  
    - Condiments & Spreads: Based on personal usage
    """, icon="ℹ️")
    else:
        st.info(f"""
    **Predicted Consumption Per Day (Crew-Wide Total)**

    - Meals (Hydratable + Thermostabilized): {CREW_SIZE * 3}  
    - Natural Form + Irradiated: {int(CREW_SIZE * 1.5)}  
    - Desserts + Beverages: {CREW_SIZE * 10}  
    - Condiments & Spreads: Based on full crew usage
    """, icon="ℹ️")
