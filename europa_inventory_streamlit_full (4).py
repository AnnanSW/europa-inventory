# europa_inventory_streamlit_full_dynamic.py
...
def modify_item(category, item):
    ...
    st.markdown(f"<div style='background-color:#f5f5f5; padding:10px; border-radius:8px; margin-bottom:5px;'>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns([3,2,2])

    with col1:
        st.markdown(f"<b style='color:{color}'>{item}</b>", unsafe_allow_html=True)
...
def apply_theme(theme_name):
    if theme_name == "Sci-Fi Europa":
        st.markdown("<style>body {background-color: #0b1d2e; color: #e0f7fa;} .stButton>button {background-color:#006064; color:#e0f7fa;}</style>", unsafe_allow_html=True)
    else:
        st.markdown("<style>body {background-color: #ffffff; color: #000000;} .stButton>button {background-color:#4CAF50; color:#ffffff;}</style>", unsafe_allow_html=True)
