...
def record_change(category, item, old_amount, new_amount, action, used_predicted=False):
    """Record change with mission day and predicted flag."""
    # Assume current mission day is the last day in change log or 1
...
def revert_version(version_id):
    try:
        # Restore exactly the chosen version's snapshot
        ...
        # Record this restoration as a new version
        st.session_state.change_log.append({
            "version_id": len(st.session_state.change_log) + 1,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "category": "SYSTEM",
...
# --- VERSION HISTORY PAGE (Updated: Mission Day Grouping, Touchscreen-Friendly) ---
def history_page():
    st.button("⬅ Back to Inventory", on_click=go_to_inventory)
    st.markdown("<hr>", unsafe_allow_html=True)

# Only show 'Back to Full History' if filtered by day or day range
    if st.session_state.get("filtered_days") and st.session_state["filtered_days"] != "all":
        if st.button("⬅ Back to Full History"):
            st.session_state["filtered_days"] = "all"
            st.rerun()


    # --- Determine current mission day ---
    # Assuming the last version reflects the latest day
    ...
    # --- Display Current Day and Last 7 Days ---
    for day in sorted_days[:8]:  # current + last 7 days
        st.markdown(f"<h3>Mission Day {day} | {len(day_groups[day])} changes</h3>", unsafe_allow_html=True)
        # Day summary: predicted vs actual consumption
        predicted_flag = any(entry.get("used_predicted", False) for entry in day_groups[day])
        if predicted_flag:
            st.success("Predicted amount used ✅")
        else:
            st.warning("Over/Under consumption ⚠️")
        ...
        for entry in day_groups[day]:
            ...
            display_text = f"V{version_id} | {timestamp} | {category} > {item} | {change_symbol}{change_amount} ({old_amount}→{new_amount})"
    ...
    if remaining_days:
        ...
        if selected_day:
            ...
            predicted_flag = any(entry.get("used_predicted", False) for entry in day_groups[selected_day])
            if predicted_flag:
                st.success("Predicted amount used ✅")
            else:
                st.warning("Over/Under consumption ⚠️")
            for entry in day_groups[selected_day]:
                version_id = entry["version_id"]
                timestamp = entry["timestamp"]
                category = entry["category"]
                item = entry["item"]
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.button("⬅ Back to Full History", on_click=history_page)
