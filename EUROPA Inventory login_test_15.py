...
def record_change(category, item, old_amount, new_amount, action):
    ...
    st.session_state.change_log.append({
        ...
        "user": st.session_state.current_user,  # ✅ NEW
        "recipients": st.session_state.get("selected_crew", []),  # ✅ NEW
        "category": category,
        "item": item,
        "action": action,
...
def edit_item_page():
    ...
    # --- NEW: Crew selection ---
    st.markdown("### Select Crew Members Receiving Item")
    ...
    selected_crew = st.multiselect(
        "Who is this for?",
        crew_list,
        default=st.session_state.get("selected_crew", [])
    )

    st.session_state.selected_crew = selected_crew
    ...
    if st.session_state.change_applied:
        ...
        if new_amount < 0:
            ...
            if action != "none":
                ...
                st.session_state.add_units_edit = 0
                st.session_state.remove_units_edit = 0
                st.session_state.selected_crew = []  # ✅ ADD THIS LINE
        ...
        for entry in reversed(day_edits[day_num]):  # Most recent edits on top
            ...
            if action.startswith("RESTORATION"):
                ...
                if recipients:
                    ...
                    crew_info = f"{user} → [{recipient_str}]"
                else:
                    crew_info = user
