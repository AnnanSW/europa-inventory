...
# --- VERSION HISTORY PAGE (Updated: Mission Day Grouping, Reverse Order, Touchscreen-Friendly) ---
def history_page():
    st.button("⬅ Back to Inventory", on_click=go_to_inventory)
    st.markdown("<hr>", unsafe_allow_html=True)
    ...
    # --- Display last 7 days + current ---
    for day in sorted_days[:8]:
        day_entries = day_groups[day]

        # --- Compute actual vs predicted per day ---
        daily_used = defaultdict(int)
        daily_predicted = defaultdict(int)

        for e in day_entries:
            if isinstance(e["old_amount"], (int,float)) and isinstance(e["new_amount"], (int,float)):
                used = e["old_amount"] - e["new_amount"]
                daily_used[e["category"]] += used
            if "predicted_units" in e:
                daily_predicted[e["category"]] += e["predicted_units"]

        # --- Calculate total per day ---
        total_used = sum(daily_used.values())
        total_predicted = sum(daily_predicted.values())

        # --- Determine flags ---
        over_flag = total_used > total_predicted * 1.02
        under_flag = total_used < total_predicted * 0.85

        st.markdown(f"<h3>Mission Day {day} | {len(day_entries)} changes</h3>", unsafe_allow_html=True)
        if over_flag:
            st.error(f"⚠️ Day's rations exceeded by {total_used - total_predicted} units:")
            for cat, used_amt in daily_used.items():
                predicted_amt = daily_predicted.get(cat,0)
                if used_amt > predicted_amt:
                    st.write(f"- {used_amt - predicted_amt} units from {cat}")
        elif under_flag:
            st.warning(f"⚠️ Day's rations below predicted by {total_predicted - total_used} units")
        else:
            st.success("Predicted amount used ✅")

        # --- Show versions in order (most recent first) ---
        for entry in sorted(day_entries, key=lambda x: x["version_id"], reverse=True):
            ...
            display_text = f"V{version_id} | {timestamp} | {category} > {item} → Restore to V{version_id} | {change_symbol}{change_amount} ({old_amount}→{new_amount})"
    ...
    if remaining_days:
        ...
        if selected_day:
            day_entries = day_groups[selected_day]
            # Repeat same display logic
            daily_used = defaultdict(int)
            daily_predicted = defaultdict(int)
            for e in day_entries:
                if isinstance(e["old_amount"], (int,float)) and isinstance(e["new_amount"], (int,float)):
                    daily_used[e["category"]] += e["old_amount"] - e["new_amount"]
                if "predicted_units" in e:
                    daily_predicted[e["category"]] += e["predicted_units"]
            ...
            st.markdown(f"<h3>Mission Day {selected_day} | {len(day_entries)} changes</h3>", unsafe_allow_html=True)
            if over_flag:
                st.error(f"⚠️ Day's rations exceeded by {total_used - total_predicted} units:")
                for cat, used_amt in daily_used.items():
                    predicted_amt = daily_predicted.get(cat,0)
