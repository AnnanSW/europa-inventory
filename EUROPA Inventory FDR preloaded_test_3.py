...
# --- HELPER FUNCTIONS ---
def is_crew_view():
    return st.session_state.get("crew_view", False) or st.session_state.current_user is None
# --- HELPER FUNCTION: GET CURRENT MISSION DAY ---
def get_current_mission_day(reference_datetime=None):
    ...
    return max(1, min(MISSION_DAYS, delta_days))  # clamp between 1 and MISSION_DAYS
...
def generate_preloaded_history():
    """
    Simulates realistic mission consumption across days:
    - multiple crew members per day
    - mixed add/remove actions
    - valid allocations
    ...
    crew_list = list(CREW_CREDENTIALS.keys())
    categories = list(inventory.keys())

    version_id = 1

    for day in range(1, min(60, MISSION_DAYS + 1)):  # simulate first 60 days
        for _ in range(random.randint(2, 6)):  # 2–6 actions per day

            category = random.choice(categories)
            item = random.choice(list(inventory[category].keys()))

            current = inventory[category][item]["current"]

            if current <= 0:
                continue

            action_type = random.choice(["remove", "remove", "add"])  # mostly consumption

            allocations = {}
            selected_crew = random.sample(crew_list, random.randint(1, 3))

            if action_type == "remove":
                qty = random.randint(1, min(5, current))

                remaining = qty

                for c in selected_crew[:-1]:
                    q = random.randint(1, max(1, remaining))
                    allocations[c] = q
                    remaining -= q

                allocations[selected_crew[-1]] = remaining
                ...
                new = min(new, inventory[category][item]["original"])

            inventory[category][item]["current"] = new

            change_log.append({
                "version_id": version_id,
                "timestamp": datetime(2026, 1, min(day, 28), 12, 0, 0).strftime("%Y-%m-%d %H:%M:%S"),
                "user": random.choice(crew_list),
                ...
                "old_amount": old,
                "new_amount": new,
                "state_after_change": copy.deepcopy(inventory),
            })

            version_id += 1

    return inventory, change_log
...
if "inventory" not in st.session_state:
    ...
    inv, log = generate_preloaded_history()

    st.session_state.inventory = copy.deepcopy(inv)
    st.session_state.change_log = copy.deepcopy(log)

    # IMPORTANT: lock initialization so it never re-runs
...
if "change_log" not in st.session_state:
    ...
    st.session_state.change_log = [{
        ...
        "old_amount": "N/A",
        "new_amount": "N/A",
        "state_before_change": copy.deepcopy(initial_inventory),
...
if "crew_view" not in st.session_state:
    st.session_state.crew_view = False
if "_usage_cache" not in st.session_state:
    st.session_state._usage_cache = {}
    
if st.session_state.get("_preloaded") and "inventory" not in st.session_state:
    inv, log = generate_preloaded_history()
    st.session_state.inventory = copy.deepcopy(inv)
    st.session_state.change_log = copy.deepcopy(log)
...
def record_change(category, item, old_amount, new_amount, action, allocations=None):
    
    st.session_state._usage_cache_reset = True
    
    st.session_state.inventory[category][item]["current"] = new_amount

    # --- UPDATE PERSONAL USAGE CACHE ---
    if allocations:
        for crew_member, qty in allocations.items():
            if crew_member not in st.session_state._usage_cache:
                st.session_state._usage_cache[crew_member] = {}

            if category not in st.session_state._usage_cache[crew_member]:
                st.session_state._usage_cache[crew_member][category] = 0
            ...
            if action == "remove":
                st.session_state._usage_cache[crew_member][category] += qty

            elif action == "add":
                st.session_state._usage_cache[crew_member][category] -= qty

                # HARD CLAMP: prevent negative usage
                if st.session_state._usage_cache[crew_member][category] < 0:
                    st.session_state._usage_cache[crew_member][category] = 0
    ...
    st.session_state.change_log.append({
        ...
        "allocations": copy.deepcopy(allocations or {}),
        "category": category,
        "item": item,
        "action": action,
        ...
        "usage_snapshot": copy.deepcopy(st.session_state._usage_cache),
    })

def revert_version(version_id):
    try:
        target_entry = next(
            entry for entry in st.session_state.change_log
            if entry["version_id"] == version_id
        )

        # Restore inventory
        restored_state = copy.deepcopy(target_entry["state_after_change"])
        st.session_state.inventory = restored_state

        # ✅ Restore personal usage PROPERLY
        st.session_state._usage_cache = copy.deepcopy(
            target_entry.get("usage_snapshot", {})
        )
        ...
        st.session_state.add_units_edit = 0
        st.session_state.remove_units_edit = 0
        st.session_state.crew_allocations = {}
        st.session_state.change_applied = False
...
# --- LOGIN PAGE ---
def login_page():
    st.markdown("<h1>EUROPA INVENTORY ACCESS</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align:center;'>CREW LOGIN</h3>", unsafe_allow_html=True)
    ...
    if st.button("Login"):
        if username in CREW_CREDENTIALS and CREW_CREDENTIALS[username] == pin:
            ...
            st.session_state.crew_view = False
            st.session_state.page = "Inventory"  # ✅ THIS LINE FIXES IT
            st.success(f"Access Granted. Welcome {username}.")
            st.rerun()
        else:
            st.error("Invalid Username or PIN")

    # --- NEW: Crew-wide stats access button ---
    if st.button("View Crew-Wide Stats"):
        st.session_state.logged_in = True
        st.session_state.current_user = None
        st.session_state.crew_view = True
        st.session_state.page = "CrewWideStats"
        st.rerun()

# --- PAGE NAVIGATION (Standard setup) ---
def go_to_inventory():
    if st.session_state.crew_view:
        st.session_state.page = "CrewWideStats"
    else:
        st.session_state.page = "Inventory"
    st.session_state.open_category = None
...
# --- CREW-WIDE STATS PAGE (Read-Only) ---
def crew_wide_stats_page():
    st.markdown("<h1>EUROPA INVENTORY: CREW-WIDE VIEW</h1>", unsafe_allow_html=True)
    st.markdown("<hr>", unsafe_allow_html=True)
    ...
    # --- Return to Login (instead of logout) ---
...
def get_personal_inventory_view(username):

    # Pull stored usage snapshot for this user
    usage_by_category = st.session_state._usage_cache.get(username, {})
    ...
    for category, items in inventory.items():
        ...
        # Total category stock split evenly across crew
        category_original_total = sum(i["original"] for i in items.values())
        category_original_person = category_original_total / CREW_SIZE

        # Get how much THIS user has used in this category
        used = usage_by_category.get(category, 0)

        # Remaining cannot go below zero
        remaining = max(category_original_person - used, 0)

        personal_data.append({
            "Category": category,
            "Original": category_original_person,
            "Used": used,
            "Remaining": remaining
        })

        total_original += category_original_person
        total_used += used

    total_remaining = max(total_original - total_used, 0)

    return personal_data, total_original, total_used, total_remaining
...
def inventory_page():
    ...
    with col_left:
        if st.session_state.current_user:
            try:
                last, first = st.session_state.current_user.split(".")
...
def dashboard_page():
    ...
    if st.session_state.crew_view:
        st.button("⬅ Back to Crew Stats", on_click=go_to_inventory)
    else:
    ...
    if total_original > 0 and DAYS_IN_MISSION > 0:
        daily_usage_rate = total_original / DAYS_IN_MISSION

        if daily_usage_rate > 0:
            overall_days_left = np.floor(total_current / daily_usage_rate)
        else:
            overall_days_left = 0
    else:
        overall_days_left = 0
    ...
    if st.session_state.current_user:
        ...
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("<h3>Personal Supply Projection</h3>", unsafe_allow_html=True)

        user = st.session_state.current_user

        pdata, p_original, p_used, p_remaining = get_personal_inventory_view(user)

        # --- PERSONAL DAILY RATE ---
        if p_original > 0:
            personal_daily_rate = p_original / DAYS_IN_MISSION
            personal_days_left = np.floor(p_remaining / personal_daily_rate) if personal_daily_rate > 0 else 0
        else:
            personal_days_left = 0

        col1, col2, col3 = st.columns(3)

        col1.metric("Your Units Remaining", f"{int(p_remaining):,}")
        col2.metric("You Have Used", f"{int(p_used):,}")
        col3.metric("Your Days Left", f"{int(personal_days_left):,}")

        st.markdown("#### Your Category Breakdown")
        ...
        for row in pdata:
            ...
            if original > 0:
                daily_rate = original / DAYS_IN_MISSION
                days_left = np.floor(remaining / daily_rate) if daily_rate > 0 else 0
            else:
                days_left = 0

            colA, colB, colC = st.columns(3)

            colA.metric("Category", cat)
            colB.metric("Remaining", f"{int(remaining):,}")
            colC.metric("Days Left", f"{int(days_left):,}")
    
    st.markdown("<br>", unsafe_allow_html=True)

    # --- Key Status Indicators (By Category) ---
    if is_crew_view():
        st.markdown("<h3>Key Status Indicators (Crew-Wide)</h3>", unsafe_allow_html=True)
    else:
        st.markdown("<h3>Key Status Indicators (Crew-Wide)</h3>", unsafe_allow_html=True)
    ...
    for _, data in df.iterrows():
        ...
        label = "Days Left (Crew)" if is_crew_view() else "Days Left (Personal)"

        col_days.metric(label, f"{int(days_rem):,}", delta=status_text, delta_color=delta_color)
...
def edit_item_page():
    ...
    # --- NEW: Crew allocation ---
    st.markdown("### Assign Items to Crew Members")

    crew_allocations = {}

    for crew in CREW_CREDENTIALS.keys():
        qty = st.number_input(
            f"{crew}",
            min_value=0,
            max_value=data["current"],
            value=0,
            key=f"crew_qty_{crew}"
        )
        if qty > 0:
            crew_allocations[crew] = qty

    st.session_state.crew_allocations = crew_allocations
    ...
    if st.session_state.change_applied:
        ...
        allocations = copy.deepcopy(st.session_state.get("crew_allocations", {}))
        ...
        if new_amount > capacity:
            st.error(f"❌ Cannot exceed max capacity ({capacity}).")
            return

        # --- ALLOCATION VALIDATION ---
        if remove_val > 0:
            if not allocations:
                st.error("❌ Must assign crew members.")
                return
            if total_allocated != remove_val:
                st.error("❌ Allocation mismatch.")
                return
            action = "remove"
        ...
        # --- APPLY CHANGES (IMPORTANT: BEFORE rerun) ---

        
        record_change(category, item, old_amount, new_amount, action, allocations)
...
""", icon="ℹ️")
    ...
    def get_mission_day(reference_datetime=None):
        if reference_datetime is None:
            reference_datetime = datetime.now()

        # safe conversion if it's a string
        if isinstance(reference_datetime, str):
            reference_datetime = datetime.strptime(reference_datetime, "%Y-%m-%d %H:%M:%S")
    ...
    for day_num in sorted(day_edits.keys(), reverse=True):
        ...
        for entry in reversed(day_edits[day_num]):  # Most recent edits on top
            ...
            if "RESTORE" in action:
                btn_label = (
                    f"V{entry['version_id']} | {timestamp} | "
                    f"SYSTEM RESTORE → V{entry.get('restored_from', '?')}"
                )
                ...
                if allocations:
                    alloc_str = ", ".join([f"{k}:{v}" for k, v in allocations.items()])
                    crew_info = f"{user} → [{alloc_str}]"
                else:
                    crew_info = user

                btn_label = f"V{entry['version_id']} | {timestamp} | {crew_info} | {category} > {item} | {change_symbol}{change_amount} ({old_amount} → {new_amount})"
...
# --- MAIN APP SWITCH (LOGIN GATE ADDED) ---
if "page" not in st.session_state:
    st.session_state.page = "Inventory"
...
if not st.session_state.logged_in:
    ...
    elif st.session_state.page == "CrewWideStats":
        crew_wide_stats_page()