# europa_inventory_streamlit_full_pretty.py
...
def modify_item(category, item):
    ...
    st.markdown(f"<div style='background-color:#f0f4f8; padding:12px; border-radius:10px; margin-bottom:8px;'>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns([3,2,2])

    with col1:
        st.markdown(f"<b style='color:{color}; font-size:16px;'>{item}</b>", unsafe_allow_html=True)
        st.progress(data['current']/data['original'])
        st.markdown(f"<small>Current: {data['current']} / Original: {data['original']}</small>", unsafe_allow_html=True)
    ...
    if data['current'] <= LOW_STOCK_THRESHOLD and data['current'] > 0:
...
def apply_theme(theme_name):
    if theme_name == "Sci-Fi Europa":
        st.markdown("<style>body {background-color: #0b1d2e; color: #e0f7fa;} .stButton>button {background-color:#006064; color:#e0f7fa; border-radius:8px;}</style>", unsafe_allow_html=True)
    else:
        st.markdown("<style>body {background-color: #ffffff; color: #000000;} .stButton>button {background-color:#4CAF50; color:#ffffff; border-radius:8px;}</style>", unsafe_allow_html=True)
