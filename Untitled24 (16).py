# europa_inventory_nasa_part1.py - Version with 6-Box Menu, Alerts, and NO Fuzzy Search
import streamlit as st
import copy
from datetime import datetime
import pandas as pd # Used for better history page display
...
def get_initial_inventory():
    # Initial inventory data remains the same
    return {
        "Hydratable Meals": {
...
# --- MAIN INVENTORY PAGE (Refactored for 6-Box Layout) ---
def inventory_page():
    ...
    for i, item_name in enumerate(menu_items):
        with cols[i % 3].container(height=120, border=True):
            if item_name != "Version History / Restore":
                st.markdown(f"### 📦 {item_name}")
                # Category Button
                if st.button("Open Category", key=f"catbtn_{item_name}", use_container_width=True):
...
def category_page():
    ...
    # Use 2 columns for a better item layout
    num_cols = 2
