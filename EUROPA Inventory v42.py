...
def history_page():
    ...
    # Determine current mission day based on total consumption
    df, total_current, total_original, _, _ = create_inventory_dataframe()
    current_mission_day = int(np.ceil(total_current / (total_original / DAYS_IN_MISSION)))

    st.markdown(f"**Current Mission Day:** {current_mission_day:,} / {DAYS_IN_MISSION}")
    st.progress(current_mission_day / DAYS_IN_MISSION)

    # Version info
    current_version_id = st.session_state.change_log[-1]["version_id"] if st.session_state.change_log else 1
    current_timestamp = st.session_state.change_log[-1]["timestamp"] if st.session_state.change_log else datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    st.markdown(f"**Current Version #:** {current_version_id} | **Time:** {current_timestamp}")
    ...
    # --- GROUP VERSIONS BY MISSION DAY ---
    # Ensure initial v1 exists
    ...
    # Filter versions by selected day range
    filtered_versions = [
        v for v in st.session_state.change_log
        if start_day <= v.get("mission_day", 1) <= end_day
    ]
    ...
    for v in filtered_versions:
        day = v.get("mission_day", 1)
        day_groups[day].append(v)

    # Determine which days to show initially: current + last 7 days
    initial_days = sorted({d for d in day_groups.keys() if d <= current_mission_day}, reverse=True)[:8]

    # Dropdown for all other days
    all_days = sorted(day_groups.keys(), reverse=True)
    remaining_days = [d for d in all_days if d not in initial_days]

    def display_day(day):
        versions = sorted(day_groups[day], key=lambda x: x["version_id"], reverse=True)  # newest first
        num_changes = len(versions)
        predicted_usage = any(v.get("action") == "PREDICTED" for v in versions)
        status_indicator = "✅ On Track" if predicted_usage else "⚠️ Over/Under"

        st.markdown(f"### Mission Day {day} | {num_changes} Changes | {status_indicator}")
        ...
        for entry in versions:
            ...
            if action.startswith("INITIAL"):
                st.markdown(f"**V{version_id}** | {timestamp} | SYSTEM INITIAL LOAD", unsafe_allow_html=True)
            else:
    ...
    # Display initial days first
    for day in initial_days:
        display_day(day)

    # Dropdown to select any other day
    if remaining_days:
        selected_day = st.selectbox("Select a mission day to view", remaining_days)
        if selected_day:
