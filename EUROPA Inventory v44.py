...
def history_page():
    ...
    # --- Show current day + last 7 days ---
    main_days = sorted_days[:8]  # current + last 7
    for day in main_days:
        ...
        for entry in reversed(day_groups[day]):
            ...
            if category == "SYSTEM" and item == "INVENTORY_WIDE":
                # Show the version it restored to
                # Extract the target version number from action string: "RESTORATION_TO_VX"
                target_version = entry["action"].split("_")[-1]  # e.g., "V3"
                display_label = f"Restored to {target_version}"
            else:
                display_label = f"{category} > {item}"

            display_text = f"V{version_id} | {timestamp} | {display_label} | {change_symbol}{change_amount} ({old_amount}→{new_amount})"
    ...
    if remaining_days:
        ...
        if selected_day:
            ...
            if predicted_flag:
                ...
                st.warning("Over/Under consumption ⚠️")
            # Versions newest to oldest
            for entry in reversed(day_groups[selected_day]):
                version_id = entry["version_id"]
                timestamp = entry["timestamp"]
                category = entry["category"]
                item = entry["item"]
