...
# --- HISTORY / CONSUMPTION RATES PAGE (FIXED BUTTONS) ---
def history_page():
    st.markdown("<h1>📊 Consumption Rates / Change Log</h1>", unsafe_allow_html=True)

    # --- Back to Inventory Button ---
    if st.button("⬅ Back to Inventory"):
        go_to_inventory()  # immediately set page
        return  # stop the function, single click now works

    st.markdown("<hr>", unsafe_allow_html=True)

    # --- Predicted Units Info (unchanged) ---
    ...
    # --- Start / End day filters (unchanged) ---
    ...
    for day in sorted_days:
        ...
        # --- Display each version for this day (newest → oldest) ---
        for entry in sorted(day_groups[day], key=lambda x: x["version_id"], reverse=True):
            ...
            if st.button(display_text, key=f"restore_{version_id}"):
                ...
                restored_state = copy.deepcopy(
                    next(e for e in st.session_state.change_log if e["version_id"] == version_id)["state_before_change"]
                )
                st.session_state.inventory = restored_state
                ...
                return  # stop function immediately so the UI refreshes correctly
