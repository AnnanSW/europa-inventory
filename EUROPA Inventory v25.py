...
def create_inventory_dataframe():
    """Converts inventory data into a Pandas DataFrame for Altair plotting and calculates key metrics.
       Removed 'Days Remaining' calculation entirely.
    """
    data_list = []
    total_original = 0
    total_current = 0
    ...
    for category, items in st.session_state.inventory.items():
        ...
        # Calculate individual item status for metrics
        ...
        data_list.append({
            ...
            # Removed Days Remaining entirely
...
def dashboard_page():
    ...
    days_per_crew_member = np.floor(overall_days_left / CREW_SIZE)
    ...
    col5.metric("Days per Crew Member", f"{int(days_per_crew_member):,} days")
    ...
    for category in df['Category']:
        ...
        # Columns for Category Metrics
        col_cat_title, col_units, col_status = st.columns([1.5, 1, 1.5])
