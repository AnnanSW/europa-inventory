...
# New: pending version restore confirmation
if "pending_restore_version" not in st.session_state:
    st.session_state.pending_restore_version = None
...
def record_change(category, item, old_amount, new_amount, action):
    st.session_state.change_log.append({
        ...
        "state_snapshot": copy.deepcopy(st.session_state.inventory)
    })
...
def revert_version(version_id):
    try:
        ...
        st.session_state.inventory = copy.deepcopy(snapshot)
        ...
        st.error("Invalid version selected.")

def confirm_restore_page():
    version = st.session_state.pending_restore_version

    st.markdown("<h2>⚠️ CONFIRM VERSION RESTORE</h2>", unsafe_allow_html=True)
    st.markdown(f"""
    You are about to revert the inventory to **Version {version}**.

    **All edits made after Version {version} will be discarded and will not be included in the new version history.**
    ...
    with col2:
        if st.button("✅ Confirm Restore"):
            ...
            st.session_state.pending_restore_version = None
            st.session_state.page = "Inventory"
            st.success(f"Inventory restored to Version {version}")
            st.rerun()
    ...
    for day_num in sorted(day_edits.keys(), reverse=True):
        ...
        for entry in reversed(day_edits[day_num]):  # Most recent edits on top
            ...
            if st.button(btn_label, key=f"restore_{entry['version_id']}"):
                st.session_state.pending_restore_version = entry["version_id"]
                st.session_state.page = "ConfirmRestore"
                st.rerun()
...
elif st.session_state.page == "ConfirmRestore":
    confirm_restore_page()
