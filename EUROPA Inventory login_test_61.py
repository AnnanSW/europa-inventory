...
def revert_version(version_id):
    try:
        ...
        # rebuild inventory from FULL HISTORY up to version
        base = copy.deepcopy(st.session_state.change_log[0]["state_after_change"])
        ...
        for entry in st.session_state.change_log:
            ...
            if entry["action"] == "remove":
                cat = entry["category"]
                item = entry["item"]
                base[cat][item]["current"] -= entry["new_amount"] - entry["old_amount"]
...
def get_personal_inventory_view(username):
    ...
    if st.session_state.get("_rebuild_all_stats"):
        st.session_state._usage_cache = {}
        st.session_state._rebuild_all_stats = False
    ...
    valid_logs = [
        ...
        and "RESTORE" not in entry["action"]
    ]
