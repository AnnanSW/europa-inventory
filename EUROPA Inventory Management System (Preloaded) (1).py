...
# --- SESSION STATE INIT WITH PRELOADED CONSUMPTION ---
...
if "change_log" not in st.session_state:
    ...
    # --- Simulate consumption over mission days ---
    current_day = get_current_mission_day()
    version_counter = 2  # start after initial snapshot
    
    for day in range(1, current_day + 1):
        # Number of changes per day
        changes_per_day = random.randint(3, 7)
        for _ in range(changes_per_day):
            category = random.choice(list(st.session_state.inventory.keys()))
            item = random.choice(list(st.session_state.inventory[category].keys()))
            old_amount = st.session_state.inventory[category][item]["current"]
            if old_amount == 0:
                continue  # skip if already zero
            
            # Random consumption: 1–5% of current stock, min 1 unit
            consumption = max(1, int(old_amount * random.uniform(0.01, 0.05)))
            new_amount = max(0, old_amount - consumption)

            # Random timestamp within that day
            change_timestamp = MISSION_START_DATE + timedelta(days=day-1,
            ...
            st.session_state.change_log.append({
                "version_id": version_counter,
                "timestamp": change_timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                "category": category,
                "item": item,
                "action": "remove",
