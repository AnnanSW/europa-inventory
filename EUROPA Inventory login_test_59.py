...
def revert_version(version_id):
    try:
        ...
        # --- SAVE CURRENT STATE (for audit log) ---
        old_state = copy.deepcopy(st.session_state.inventory)

        # --- RESTORE INVENTORY SNAPSHOT ---
        st.session_state.inventory = copy.deepcopy(target_entry["state_after_change"])

        # --- TIMELINE / TRACKING ---
        st.session_state.timeline_head = version_id
        st.session_state._last_restore_version = version_id

        # --- RESET CACHE FLAGS ---
        st.session_state._usage_cache_reset = True
        st.session_state._usage_cache = {}

        # --- CLEAN INVENTORY SAFETY ---
        ...
        # --- 🔥 CRITICAL FIX: WIPE DYNAMIC WIDGET STATE ---
        # removes stale number_inputs from edit page + crew allocations
        keys_to_delete = []
        for key in list(st.session_state.keys()):
            if (
                key.startswith("crew_qty_") or
                key.startswith("add_units_input") or
                key.startswith("remove_units_input") or
                key == "barcode_input"
            ):
        ...
        # --- OPTIONAL: CLEAR FORM STATE SAFELY ---
        # prevents stuck form submissions after restore
        if "barcode_scan_form" in st.session_state:
            del st.session_state["barcode_scan_form"]
...
def get_personal_inventory_view(username):
    if st.session_state.get("_usage_cache_reset"):
        st.session_state._usage_cache = {}
        st.session_state._usage_cache_reset = False
        st.session_state.pop("_personal_usage_cache", None)
    logs = st.session_state.change_log

    # ---------------------------------------------------
    # STEP 1: determine valid log window (restore-safe)
    # ---------------------------------------------------

    last_restore_version = getattr(st.session_state, "_last_restore_version", None)

    # Use actual logs
    head = st.session_state.get("timeline_head", None)

    valid_logs = []

    for entry in st.session_state.change_log:

        if entry["action"] == "INITIALIZATION":
            continue

        # 🚨 THIS is the restore cutoff logic (Step 2 you added)

        # 🚨 Step 3 (important addition)
    ...
    # reset cache if restore happened
    if st.session_state.get("_usage_cache_reset"):
        st.session_state._usage_cache = {}
        st.session_state._usage_cache_reset = False
    ...
    for entry in valid_logs:
        ...
        # prevent double counting restored chains
        ...
        for crew_member, qty in allocations.items():
            if crew_member != username:
                continue

            # REMOVE = crew consumed inventory
            if action == "remove":
                usage_by_category[category] = usage_by_category.get(category, 0) + qty

            # ADD = crew returned/restocked inventory (reverse usage)
            elif action == "add":
                usage_by_category[category] = usage_by_category.get(category, 0) - qty
        
    # ---------------------------------------------------
    # STEP 3: compute totals from current inventory state
    # ---------------------------------------------------
    ...
    return personal_data, total_original, total_used, max(total_original - total_used, 0)
