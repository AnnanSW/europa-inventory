...
def get_personal_inventory_view(username):
    ...
    # 1. accumulate only THIS user's consumption actions
    for entry in logs:
        if entry.get("user") != username:
            continue

        if entry["action"] not in ["add", "remove"]:
            continue
        ...
        change = entry["new_amount"] - entry["old_amount"]

        # only count consumption as positive usage
        if entry["action"] == "remove":
            personal_usage[category] = personal_usage.get(category, 0) + abs(change)
    ...
    for category, items in inventory.items():
        ...
        used = personal_usage.get(category, 0)
