...
def inventory_page():
    ...
    # --- Current Mission Day Indicator (styled, compact) ---
    ...
    st.markdown(
        f"**Mission Day:** {current_day} / {MISSION_DAYS} &nbsp;&nbsp;|&nbsp;&nbsp; "
        f"**Days Remaining:** {days_left}"
    )
    st.progress(current_day / MISSION_DAYS)
    st.markdown("<hr>", unsafe_allow_html=True)

    # --- BARCODE SCANNER (AUTOFOCUSED ON LOAD & NAVIGATION) ---
    with st.form("barcode_scan_form", clear_on_submit=True):
        scanned_barcode = st.text_input(
            "SCAN BARCODE",
            key="barcode_input",
            help="Scan a product barcode here. Cursor auto-focuses on page load.",
            autofocus=True  # 👈 THIS IS THE KEY LINE
        )
    ...
    with col_nav1:
        if st.button(
            "📊 Inventory Dashboard",
            help="View mission-wide inventory status",
            use_container_width=True
        ):
            go_to_dashboard()
    ...
    with col_nav2:
        if st.button(
            "📚 Consumption History",
            help="View and restore historical consumption changes",
            use_container_width=True
        ):
