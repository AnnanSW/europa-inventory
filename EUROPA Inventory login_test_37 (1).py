...
def revert_version(version_id):
    try:
        ...
        # APPLY FIRST (important: state must exist before logging)
        st.session_state.inventory = copy.deepcopy(restored_state)

        # REMOVE ONLY FUTURE NON-RESTORE LOGS (KEEP RESTORE HISTORY CLEAN)
        st.session_state.change_log = [
            entry for entry in st.session_state.change_log
            if entry["version_id"] <= version_id
            or entry.get("action", "").startswith("RESTORATION")
        ]

        # NOW log the restoration as a NEW version
        st.session_state.change_log.append({
            "version_id": len(st.session_state.change_log) + 1,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "category": "SYSTEM",
