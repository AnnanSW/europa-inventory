...
def settings_page():
    ...
    if st.button("✅ Apply & Recalculate Inventory"):
        # --- Record old values for versioning ---
        old_crew = st.session_state.crew_size
        old_days = st.session_state.mission_days
        old_redundancy = st.session_state.redundancy_factor
        ...
        st.session_state.change_log.append({
            ...
            "action": "update",
            "old_amount": f"Crew={old_crew}, Days={old_days}, Redundancy={old_redundancy}",
            "new_amount": f"Crew={crew}, Days={days}, Redundancy={redundancy}",
            "state_before_change": copy.deepcopy(st.session_state.inventory)
        })
        ...
        # --- Wait 3 seconds and return to Inventory page ---
        time.sleep(3)
        go_to_inventory()
    ...
    if st.session_state.change_applied:
        ...
        if new_amount < 0:
            ...
            if action != "none":
                ...
                st.success(f"✅ **UPDATE SUCCESS** | {item} is now at **{new_amount}** units.")
                time.sleep(3)  # wait 3 seconds
                go_to_inventory()
                st.experimental_rerun()
            else:
