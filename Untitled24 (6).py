# europa_inventory_part1_fixed.py
...
def go_back_to_inventory():
    ...
    st.session_state.open_category = None
    st.session_state.edit_item_page = None
# europa_inventory_part2_fixed.py
import streamlit as st

# --- PAGE RENDERING FUNCTIONS ---
def render_inventory_page():
    st.title("EUROPA INVENTORY SYSTEM")
    st.write("Select a category to view items:")
...
def render_category_page():
    ...
    st.title(f"{category.upper()}")
    st.write("Click on a food item to view/edit details:")
    for item, data in st.session_state.inventory[category].items():
        item_color = get_item_color(data["current"])
        display_text = f"{item} — Current: {data['current']} / Original: {data['original']}"
        if st.button(display_text, key=f"{category}-{item}"):
...
def render_item_page():
    ...
    st.title(f"{item.upper()} ({category})")
    st.write(f"Original: {data['original']} | Current: {data['current']}")
    
    col1, col2 = st.columns(2)
    
    with col1:
        add_val = st.number_input("Add Units", min_value=0, value=st.session_state.add_units_edit, key="add_units")
        if add_val > 0:
            st.session_state.remove_units_edit = 0
    
    with col2:
        remove_val = st.number_input("Remove Units", min_value=0, value=st.session_state.remove_units_edit, key="remove_units")
        if remove_val > 0:
            st.session_state.add_units_edit = 0
    ...
    if st.button("Apply Changes"):
        ...
        if new_amount > data["original"]:
            st.error(f"Cannot exceed original capacity ({data['original']})")
        elif new_amount < 0:
            st.error("Cannot remove more than current amount")
        else:
            ...
            if action != "none":
                ...
                st.success(f"Updated {item}: {old_amount} → {new_amount}")
        st.session_state.add_units_edit = 0
        st.session_state.remove_units_edit = 0
...
def render_version_history_page():
    ...
    for entry in reversed(st.session_state.change_log):
        ...
        item = entry["item"]
        action = entry["action"]
        old_amt = entry["old_amount"]
        new_amt = entry["new_amount"]
        st.write(f"{timestamp} — {category} / {item}: {action} ({old_amt} → {new_amt})")
    if st.button("⬅ Back"):
        go_back_to_inventory()
