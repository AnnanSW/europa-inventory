...
def revert_version(version_id):
    try:
        ...
        # --- TRIM HISTORY TO RESTORED VERSION ---
        cut_index = next(
            (i for i, entry in enumerate(st.session_state.change_log)
            if entry["version_id"] == version_id),
            None
        )
        ...
        st.session_state.change_log = st.session_state.change_log[:cut_index + 1]
...
def get_personal_inventory_view(username):
    ...
    # Use actual logs
    valid_logs = st.session_state.change_log
