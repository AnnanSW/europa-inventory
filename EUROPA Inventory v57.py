...
def get_initial_inventory():
    ...
    for category, (items, multipliers) in category_items.items():
        ...
        # Adjust for redundancy factor and mission duration
        uniform_units = int(np.ceil(np.mean(calculated_units) * REDUNDANCY_FACTOR))
...
def create_inventory_dataframe():
    ...
    for category, items in st.session_state.inventory.items():
        ...
        data_list.append({
            ...
            # --- Calculate days remaining using daily predicted usage including redundancy ---
            'Days Remaining': np.floor(cat_current / ((CREW_SIZE*3*REDUNDANCY_FACTOR) if category in ["Hydratable Meals","Thermostabilized Meals"] 
                                         else (CREW_SIZE*1.5*REDUNDANCY_FACTOR) if category=="Natural Form & Irradiated"
                                         else (CREW_SIZE*8*REDUNDANCY_FACTOR) if category=="Desserts & Beverages (Non-Mix)"
                                         else 1)
                           ) if cat_current > 0 else np.inf,
...
def history_page():
    ...
    if st.button("⬅ Back to Inventory"):
        st.session_state.page = "Inventory"
        return  # safely exit, Streamlit will rerun the next cycle


    st.markdown("<hr>", unsafe_allow_html=True)

    # --- Predicted Units Banner (Always Visible) ---
    predicted_units_per_day = {
        ...
        "Desserts & Beverages (Non-Mix)": CREW_SIZE * 8,
        "Condiments & Spreads": "Specific to Entree, Snack, or Dessert"
    }

    st.info(
        f"Predicted daily consumption for the crew:\n"
        f"- Entrées (Hydratable + Thermostabilized): {predicted_units_per_day['Hydratable Meals'] + predicted_units_per_day['Thermostabilized Meals']} units/day\n"
        f"- Natural Form & Irradiated: {predicted_units_per_day['Natural Form & Irradiated']} units/day\n"
        f"- Desserts & Beverages: {predicted_units_per_day['Desserts & Beverages (Non-Mix)']} units/day\n"
        f"- Condiments & Spreads: {predicted_units_per_day['Condiments & Spreads']}",
        icon="ℹ️"
    )
    ...
    with col_end:
        end_day = st.number_input("End Day", min_value=1, max_value=DAYS_IN_MISSION, value=min(7, DAYS_IN_MISSION))
    ...
    sorted_days = sorted(day_groups.keys(), reverse=True)  # Newest first
    ...
    for day in sorted_days:
        ...
        # --- Daily Consumption Banner ---
        category_daily_consumption = defaultdict(int)
        for entry in day_groups[day]:
            ...
            if isinstance(old_amount, int) and isinstance(new_amount, int):
                category_daily_consumption[category] += old_amount - new_amount

        st.info(
            f"Consumption summary for the day:\n"
            f"- Entrées consumed: {category_daily_consumption.get('Hydratable Meals',0) + category_daily_consumption.get('Thermostabilized Meals',0)} units\n"
            f"- Natural Form & Irradiated consumed: {category_daily_consumption.get('Natural Form & Irradiated',0)} units\n"
            f"- Desserts & Beverages consumed: {category_daily_consumption.get('Desserts & Beverages (Non-Mix)',0)} units\n"
            f"- Condiments & Spreads consumed: {category_daily_consumption.get('Condiments & Spreads',0)} units",
            icon="📊"
        )

        # --- Display each version for this day ---
        for entry in sorted(day_groups[day], key=lambda x: x["version_id"], reverse=True):  # Newest version on top
            ...
            # Numeric change display
            if isinstance(old_amount, int) and isinstance(new_amount, int):
                change_str = f"{old_amount} → {new_amount}"
            else:
                change_str = ""
            ...
            if st.button(display_text, key=f"restore_{version_id}"):
                # 1️⃣ Find the snapshot for the version to restore
                try:
                    restored_state = copy.deepcopy(
                        next(entry for entry in st.session_state.change_log if entry["version_id"] == version_id)["state_before_change"]
                    )
                ...
                # 2️⃣ Restore inventory to that snapshot
                st.session_state.inventory = restored_state

                # 3️⃣ Record this restoration as a new version in the log
                st.session_state.change_log.append({
                    "version_id": len(st.session_state.change_log) + 1,
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "category": "SYSTEM",
                    "item": "RESTORE",
                    "action": f"RESTORED_TO_V{version_id}",
                    "old_amount": "N/A",
                    "new_amount": "N/A",
                    "state_before_change": copy.deepcopy(st.session_state.inventory),
                    "mission_day": st.session_state.change_log[-1].get("mission_day", 1)
                ...
                # 4️⃣ Optional flag for updates
                st.session_state.restore_version_id = version_id

                # 5️⃣ Exit safely so Streamlit reruns naturally
                return
