...
def login_page():
    ...
    if st.button("View Crew-Wide Stats"):
        st.session_state.logged_in = True
        st.session_state.current_user = None  # No personal login
...
def crew_wide_stats_page():
    ...
    for _, data in df.iterrows():
        ...
        col_days.metric("Days Left (All Crew)", f"{int(days_rem):,}")
        col_status.metric("Consumption %", f"{data['Consumption Rate']:.1%}", delta=f"Used: {data['Consumed Stock']:,}")
