    ...
    for entry in reversed(st.session_state.change_log):
        ...
        if action.startswith("RESTORATION"):
            ...
            if action == "INITIALIZATION":
                st.markdown(f"**V{version_id}** | {timestamp} | INITIAL INVENTORY LOADED", unsafe_allow_html=True)
            else:
            ...
            # Restore button now restores to **exact version**, even V1
            if st.button(f"Restore to V{version_id}", key=f"restore_{version_id}"):
                revert_version(version_id)
                st.success(f"Inventory restored to V{version_id}")
                st.rerun()
