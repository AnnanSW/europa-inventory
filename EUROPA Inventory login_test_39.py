...
def revert_version(version_id):
    try:
        ...
        # IMPORTANT FIX: preserve lineage clarity
        st.session_state.change_log.append({
            "version_id": len(st.session_state.change_log) + 1,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "user": st.session_state.current_user,
            "category": "SYSTEM",
            "item": "INVENTORY_WIDE",
            "action": f"RESTORE_V{version_id}",  # FIXED (no ambiguity)
            "old_amount": "N/A",
            "new_amount": "N/A",
            "state_before_change": old_state,
            "state_after_change": restored_state,
            "restored_from": version_id  # 🔥 KEY ADDITION
        })
...
def get_personal_inventory_view(username):
    """
    Builds a personal inventory view based on:
    - Equal initial share (original / crew size)
    - Actual usage from allocation logs
    """
    ...
    for category, items in inventory.items():
        ...
        for entry in logs:
            ...
            if entry.get("action") == "remove":
                ...
                category_used += alloc.get(username, 0)
    

        category_remaining = category_original_person - category_used
