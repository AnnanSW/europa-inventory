...
def inject_button_styles(category=None, item_name=None, color=None):
    ...
    if category and item_name and color:
        ...
        # We use a semi-transparent dark background for the button body
        css = f"""
        <style>
        ...
        # Use Red for Critical/Zero, Amber for Warning, White for Default
...
# --- CUSTOM PROFESSIONAL HUD THEME STYLES (Finalized) ---
st.markdown("""
<style>
...
h1 {
    color: #00AEEF !important; /* Electric Blue Accent */
    text-align: center;
    border-bottom: 3px solid #00AEEF;
    padding-bottom: 10px;
    letter-spacing: 8px;
    text-shadow: 0 0 15px rgba(0, 174, 239, 0.9); /* Strong Glow */
...
div[data-testid="stNumberInput"] label {
    color: #00AEEF !important; /* Blue label for data input */
}
div[data-testid="stNumberInput"] input {
    color: #FAFAFA; 
    background-color: #0A192F; 
    border: 1px solid #00AEEF; /* Blue input border */
    padding: 5px;
    border-radius: 3px;
}
...
div.stButton > button {
    ...
    border: 1px solid #303C54; /* Subtler default border */
    border-radius: 0px; 
    padding: 10px 20px;
...
div.stButton > button:hover {
    ...
    border-color: #00AEEF; /* Blue hover */
}

/* 6. Alerts */
div[data-testid="stAlert"].stAlert-success {
    background-color: #002244; /* Dark Blue Success */
    border-color: #00AEEF;
...
div[data-testid="stAlert"].stAlert-warning {
    background-color: #382A15; /* Dark Orange Warning */
    border-color: #FFB703; /* Amber border */
}

/* 7. Progress Bar */
.stProgress > div > div > div > div {
    background-color: #00AEEF; /* Electric Blue bar color */
    box-shadow: 0 0 5px #00AEEF;
}

/* 8. Horizontal Rules */
hr {
    border-top: 2px solid #0F375F; /* Subtle Navy Divider */
    margin-top: 15px;
    margin-bottom: 15px;
}
...
def get_category_color(category):
    ...
    for item in st.session_state.inventory[category].values():
        ...
        if item["current"] <= LOW_STOCK_THRESHOLD:
            color = "#FFB703" # Amber is the warning
    return color
...
# --- MAIN INVENTORY PAGE ---
def inventory_page():
    st.markdown("<p style='color:#00AEEF; text-align:center; letter-spacing: 2px;'>**SYSTEM STATUS: DATA STREAM ACTIVE**</p>", unsafe_allow_html=True)
...
<div style='position: fixed; bottom: 0; left: 0; width: 100%; 
            background-color: #0F375F; color: #00AEEF; 
            padding: 5px 10px; font-size: 10px; 
            border-top: 1px solid #00AEEF; letter-spacing: 1px;'>
    EUROPA INVENTORY SYSTEM V1.3.0 | STATUS: NOMINAL | UPTIME: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} | DATA HASH: {hash(str(st.session_state.inventory)) % 1000000}
</div>
""", unsafe_allow_html=True)