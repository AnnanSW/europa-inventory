...
def get_initial_inventory():
    """
    Calculates initial stock based on 7 crew members for a 650-day Mars mission
    with 30% NASA redundancy.
    Condiments are excluded from caloric intake and scaled proportionally.
    """

    # --- DAILY FOOD ASSUMPTION ---
    # ~3 meals + snacks per astronaut per day
    DAILY_UNITS_PER_PERSON = 4.5

    total_food_units_needed = int(
        np.ceil(CREW_SIZE * DAYS_IN_MISSION * DAILY_UNITS_PER_PERSON)
    )

    # Number of NON-condiment food items
    NON_CONDIMENT_CATEGORIES = [
        "Hydratable Meals",
    ...
    non_condiment_items = [
        v for v in BARCODE_MAP.values()
        if v[0] in NON_CONDIMENT_CATEGORIES
    ]

    units_per_item_base = total_food_units_needed / len(non_condiment_items)

    def calculate_stock(multiplier):
        return int(np.ceil(units_per_item_base * multiplier))
    ...
    # Condiments are scaled later
    stock_cs = [1.0] * 10

    items_hm = [v[1] for v in BARCODE_MAP.values() if v[0] == "Hydratable Meals"]
    items_tm = [v[1] for v in BARCODE_MAP.values() if v[0] == "Thermostabilized Meals"]
    items_nf = [v[1] for v in BARCODE_MAP.values() if v[0] == "Natural Form & Irradiated"]
    items_db = [v[1] for v in BARCODE_MAP.values() if v[0] == "Desserts & Beverages (Non-Mix)"]
    items_cs = [v[1] for v in BARCODE_MAP.values() if v[0] == "Condiments & Spreads"]

    inventory_data = {
        "Hydratable Meals": {
            n: {"current": calculate_stock(m), "original": calculate_stock(m)}
            for n, m in zip(items_hm, stock_hm)
        },
        "Thermostabilized Meals": {
            n: {"current": calculate_stock(m), "original": calculate_stock(m)}
            for n, m in zip(items_tm, stock_tm)
        },
        "Natural Form & Irradiated": {
            n: {"current": calculate_stock(m), "original": calculate_stock(m)}
            for n, m in zip(items_nf, stock_nf)
        },
        "Desserts & Beverages (Non-Mix)": {
            n: {"current": calculate_stock(m), "original": calculate_stock(m)}
            for n, m in zip(items_db, stock_db)
        }
    }

    # --- CONDIMENT SCALING (NON-CALORIC) ---
    total_food_units = sum(
        item["original"]
        for cat in inventory_data.values()
        for item in cat.values()
    )

    # Rough rule: 1 condiment unit per 12 food units
    condiment_base = int(np.ceil(total_food_units / 12 / len(items_cs)))

    inventory_data["Condiments & Spreads"] = {
        n: {
            "current": condiment_base,
            "original": condiment_base
        }
        for n in items_cs
    }
