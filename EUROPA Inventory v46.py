...
# --- CONFIGURATION ---
LOW_STOCK_THRESHOLD = 30 # Increased threshold to reflect larger stock numbers
WARNING_STOCK_THRESHOLD = 60

# --- MISSION PARAMETERS FOR STOCK CALCULATION ---
CREW_SIZE = 7
MISSION_DAYS = 1035
REDUNDANCY_FACTOR = 1.30  # 30% NASA-style margin
...
def get_initial_inventory():
    """
    NASA-sized portion modeling:
    - Entrées = 1.0 meal
    - Sides/snacks = 0.5 meal
    - Desserts/beverages = 0.25 meal
    - Condiments scaled proportionally
    """

    MEALS_PER_PERSON_PER_DAY = 3
    REDUNDANCY_FACTOR = 1.30

    # Mission duration (use DAYS_IN_MISSION from your config)
    total_meals_required = int(
        np.ceil(CREW_SIZE * MEALS_PER_PERSON_PER_DAY * DAYS_IN_MISSION * REDUNDANCY_FACTOR)
    )

    # Meal-equivalent weight per category
    MEAL_WEIGHTS = {
    ...
    total_weighted_units = sum(
        len(items) * MEAL_WEIGHTS[cat]
        for cat, (items, _) in category_items.items()
    )

    # Base units per weight
    base_units_per_weight = total_meals_required / total_weighted_units

    # --- Inventory dictionary ---
    inventory_data = {}

    # Assign uniform stock per category
    for category, (items, multipliers) in category_items.items():
        meal_weight = MEAL_WEIGHTS[category]

        # Original per-item calculation
        calculated_units = [base_units_per_weight * meal_weight * mult for mult in multipliers]

        # Ceiling of average → uniform units
        uniform_units = int(np.ceil(np.mean(calculated_units)))

        # Assign same number to every item in the category
        inventory_data[category] = {
            name: {"current": uniform_units, "original": uniform_units}
            for name in items
        }

    # --- Condiments ---
    # Condiments should scale roughly 1 per entrée pouch
    total_entrees = sum(
        item["original"]
        for cat in ["Hydratable Meals", "Thermostabilized Meals"]
        for item in inventory_data[cat].values()
    )
    ...
    inventory_data["Condiments & Spreads"] = {
        name: {"current": per_condiment, "original": per_condiment}
        for name in condiment_items
    }
...
def record_change(category, item, old_amount, new_amount, action, used_predicted=False, predicted_units_per_day=None):
    """Record a change with mission day and predicted flag."""
    # Current mission day = last version or 1
    current_mission_day = st.session_state.change_log[-1].get("mission_day", 1) if st.session_state.change_log else 1

    # If predicted_units_per_day not provided, use old value or default to 0
...
def revert_version(version_id):
    try:
        ...
        restored_entry = next(entry for entry in st.session_state.change_log if entry["version_id"] == version_id)
        restored_state = copy.deepcopy(restored_entry["state_before_change"])
        st.session_state.inventory = restored_state

        # Record restoration as a new version
        new_version_id = len(st.session_state.change_log) + 1
        st.session_state.change_log.append({
            "version_id": new_version_id,
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "category": "SYSTEM",
            "item": f"RESTORE_TO_V{version_id}",
            "action": "RESTORE",
            "old_amount": "N/A",
            "new_amount": "N/A",
            "state_before_change": copy.deepcopy(st.session_state.inventory),
            "mission_day": restored_entry["mission_day"],
            "used_predicted": True
        })

        # --- Recalculate future days after restored day ---
        restored_day = restored_entry["mission_day"]
        for i, entry in enumerate(st.session_state.change_log):
            if entry.get("mission_day", 0) > restored_day:
                # Recalculate new_amounts from current inventory snapshot
                cat = entry["category"]
                itm = entry["item"]
                if cat in st.session_state.inventory and itm in st.session_state.inventory[cat]:
                    entry["old_amount"] = st.session_state.inventory[cat][itm]["current"]
                    entry["new_amount"] = entry["old_amount"]  # neutralize, will be applied by future changes
                # Optional: you could adjust predicted flags here if needed
...
# --- DASHBOARD PAGE (Retained) ---
# --- DASHBOARD PAGE (Updated: No Days per Crew Member) ---
def dashboard_page():
    ...
    overall_days_left = np.floor(
        total_current / (total_original / DAYS_IN_MISSION)
    )

    # --- Global Metrics Section ---
    st.markdown("<h3>Mission-Wide Status</h3>", unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)  # Removed 5th column for per crew member
    ...
    for _, data in df.iterrows():
        ...
        # Adjust columns: removed "Days per Crew" column
...
# --- VERSION HISTORY PAGE (Updated: Mission Day Grouping, Predicted Usage, Flags, Restore Updates) ---
def history_page():
    st.markdown("<h2>// VERSION HISTORY</h2>", unsafe_allow_html=True)
    st.button("⬅ Back to Inventory", on_click=go_to_inventory)
    st.markdown("<hr>", unsafe_allow_html=True)

    # --- Determine current mission day ---
    if st.session_state.change_log:
        current_day = st.session_state.change_log[-1].get("mission_day", 1)
        current_version = st.session_state.change_log[-1]["version_id"]
        current_time = st.session_state.change_log[-1]["timestamp"]
    else:
        current_day = 1
        current_version = 1
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    st.markdown(f"<b>Current Mission Day:</b> {current_day}", unsafe_allow_html=True)
    st.progress(current_day / DAYS_IN_MISSION)
    st.markdown(f"<b>Current Version:</b> V{current_version} | <b>Time:</b> {current_time}", unsafe_allow_html=True)

    # --- Day Range Selector ---
    col_start, col_end = st.columns(2)
    with col_start:
        start_day = st.number_input("Start Day", min_value=1, max_value=DAYS_IN_MISSION, value=1)
    with col_end:
        end_day = st.number_input("End Day", min_value=1, max_value=DAYS_IN_MISSION, value=DAYS_IN_MISSION)

    # --- Filter change log by selected range ---
    filtered_log = [entry for entry in st.session_state.change_log
                    if start_day <= entry.get("mission_day", 1) <= end_day]

    if not filtered_log:
        st.info("No changes recorded in this day range.")
        return

    # --- Group by mission day ---
    from collections import defaultdict
    day_groups = defaultdict(list)
    for entry in filtered_log:
        day_groups[entry.get("mission_day", 1)].append(entry)

    sorted_days = sorted(day_groups.keys(), reverse=True)

    # --- Precompute predicted vs actual per day ---
    # You may have predicted units per day stored elsewhere; for this example assume 1 unit per item per day as placeholder
    PREDICTED_PER_DAY = {}
    for day, entries in day_groups.items():
        day_predicted = 0
        day_used = 0
        category_diff = defaultdict(int)
        for entry in entries:
            if entry.get("old_amount") != "N/A" and entry.get("new_amount") != "N/A":
                used_units = entry["old_amount"] - entry["new_amount"] if entry["old_amount"] >= entry["new_amount"] else entry["new_amount"] - entry["old_amount"]
                day_used += used_units
                category_diff[entry["category"]] += used_units
            day_predicted += 1  # placeholder: increment for predicted logic (replace with actual per-day predictions)
        PREDICTED_PER_DAY[day] = {"predicted": day_predicted, "used": day_used, "category_diff": category_diff}

    # --- Top Table: Predicted vs Actual ---
    st.markdown("<h3>Predicted vs Actual Usage Summary</h3>", unsafe_allow_html=True)
    summary_data = []
    for day in sorted_days:
        info = PREDICTED_PER_DAY[day]
        summary_data.append({
            "Mission Day": day,
            "Predicted Units": info["predicted"],
            "Actual Used": info["used"],
            "Difference": info["used"] - info["predicted"]
        })
    st.table(pd.DataFrame(summary_data))

    # --- Display Current Day + Last 7 Days ---
    for day in sorted_days[:8]:
        st.markdown(f"<h3>Mission Day {day} | {len(day_groups[day])} changes</h3>", unsafe_allow_html=True)

        # --- Determine over/under per category ---
        category_over_under_texts = []
        info = PREDICTED_PER_DAY[day]
        for cat, units_used in info["category_diff"].items():
            predicted_units = 1  # placeholder; replace with actual predicted per category
            diff_units = units_used - predicted_units
            if diff_units > 0:
                category_over_under_texts.append(f"{diff_units} units over predicted ({cat})")
            elif diff_units < 0:
                category_over_under_texts.append(f"{-diff_units} units under predicted ({cat})")

        # --- Flag logic ---
        if info["used"] == 0:
            st.info("No usage recorded yet ⚠️")
        else:
            total_pred = info["predicted"]
            diff_pct = (info["used"] - total_pred) / total_pred
            if diff_pct > 0.02:
                st.error(f"Day's rations exceeded by {info['used'] - total_pred} units | " + ", ".join(category_over_under_texts))
            elif diff_pct < -0.15:
                st.warning(f"Day's rations underused by {total_pred - info['used']} units | " + ", ".join(category_over_under_texts))
            else:
                st.success(f"Predicted amount used ✅ | " + ", ".join(category_over_under_texts))

        # --- Display versions for the day (top to bottom) ---
        for entry in sorted(day_groups[day], key=lambda x: -x["version_id"]):
            ...
            # Use option 2: show which version it restores to
            if action.startswith("RESTORATION_TO_V"):
                restored_to = action.split("_")[-1]
                action_display = f"RESTORED TO V{restored_to}"
            else:
                action_display = action

            change_symbol = "+" if action == "add" else ("-" if action == "remove" else "")
            change_amount = abs(new_amount - old_amount) if old_amount != "N/A" else "N/A"

            display_text = f"V{version_id} | {timestamp} | {category} > {item} | {action_display} {change_symbol}{change_amount} ({old_amount}→{new_amount})"

            if st.button(display_text, key=f"restore_{version_id}"):
                revert_version(version_id)
                st.success(f"Inventory restored to state at V{version_id}")
                st.rerun()

        st.markdown("<hr>", unsafe_allow_html=True)

    # --- Dropdown to select other days ---
    remaining_days = sorted_days[8:] if len(sorted_days) > 8 else []
    if remaining_days:
        selected_day = st.selectbox("Select Another Day to View", remaining_days)
        if selected_day:
            st.markdown(f"<h3>Mission Day {selected_day} | {len(day_groups[selected_day])} changes</h3>", unsafe_allow_html=True)
            info = PREDICTED_PER_DAY[selected_day]
            category_over_under_texts = []
            for cat, units_used in info["category_diff"].items():
                predicted_units = 1
                diff_units = units_used - predicted_units
                if diff_units > 0:
                    ...
                    category_over_under_texts.append(f"{-diff_units} units under predicted ({cat})")
            diff_pct = (info["used"] - info["predicted"]) / info["predicted"]
            if info["used"] == 0:
                st.info("No usage recorded yet ⚠️")
            elif diff_pct > 0.02:
                st.error(f"Day's rations exceeded by {info['used'] - info['predicted']} units | " + ", ".join(category_over_under_texts))
            elif diff_pct < -0.15:
                st.warning(f"Day's rations underused by {info['predicted'] - info['used']} units | " + ", ".join(category_over_under_texts))
            else:
                st.success(f"Predicted amount used ✅ | " + ", ".join(category_over_under_texts))

            for entry in sorted(day_groups[selected_day], key=lambda x: -x["version_id"]):
                version_id = entry["version_id"]
                timestamp = entry["timestamp"]
                category = entry["category"]
                item = entry["item"]
                ...
                display_text = f"V{version_id} | {timestamp} | {category} > {item} | {action_display} {change_symbol}{change_amount} ({old_amount}→{new_amount})"
                if st.button(display_text, key=f"restore_{version_id}_dropdown"):
                    revert_version(version_id)
                    st.success(f"Inventory restored to state at V{version_id}")
                    st.rerun()

# --- MAIN APP SWITCH (Retained) ---
